...
// DB-sync matrix when ISRC is missing on one side. Same approach as the
// batch-enrich endpoint: normalize titles to alphanumerics-only, lowercase,
// compute Levenshtein distance, return a 0…1 similarity score.
...
// ── Batch Enrich: enrich multiple catalogues from a single Excel upload ───────
// Body: { rows: ExcelRow[] }  — all rows from the spreadsheet, spanning any
// number of catalogues.  Groups rows by catalogue field, then for each group
// fetches FM records and applies the same three-pass match + PATCH logic.
router.post('/batch-enrich', adminAuth, express.json(), async (req, res) => {
  const { rows } = req.body || {}
  if (!Array.isArray(rows) || !rows.length) {
    return res.status(400).json({ error: 'rows array required' })
  }

  // Group Excel rows by catalogue number
  const groups = new Map()
  for (const row of rows) {
    const cat = (row.catalogue || '').trim()
    if (!cat) continue
  ...
  if (!groups.size) {
    return res.status(400).json({ error: 'No catalogue numbers found in Excel rows — check the Catalogue column is mapped correctly' })
  }

  const lf = await resolveGalloLineFields()

  // Shared field-name helper (same as single enrich)
  function buildFieldDataBatch(row) {
    const f = {}
    if (row.title)        f['Track Name']                 = row.title
    if (row.artist_name)  f['Track Artist']               = row.artist_name
    if (row.album_artist) f['Album Artist']               = row.album_artist
    ...
    if (row.p_line && lf.pName) f[lf.pName]               = row.p_line
    if (row.c_line && lf.cName) f[lf.cName]               = row.c_line
    applyCcaRules(f, row, row.catalogue, lf)
    return f
  }
  ...
  for (const [cat, excelRows] of groups) {
    const catResult = { catalogue: cat, matched: 0, errors: [], unmatched_fm: [], unmatched_xl: [] }

    let fmTracks
    try {
      fmTracks = await findGalloRecordsByCatalogue(cat)
    } catch (e) {
      catResult.errors.push({ title: '(FM query)', error: e.message })
      catalogueResults.push(catResult)
      continue
    }

    if (!fmTracks.length) {
      catResult.errors.push({ title: '(FM query)', error: `No FM records found for "${cat}"` })
      catalogueResults.push(catResult)
      continue
    }

    // Extract track number from audio container filename prefix
    function extractTrackNum(containerUrl) {
      if (!containerUrl) return null
      const filename = containerUrl.startsWith('movie:')
        ? containerUrl.split('\r')[0].replace('movie:', '').trim()
    ...
    // Three-pass matching (same logic as frontend + single enrich)
    const usedExcel = new Set()
    const pairs = []

    // Pass 1 — track number
    for (const fm of fmTracks) {
      const fmNum = extractTrackNum(fm.audio_container_url) ?? fm.sequence_no
      if (fmNum == null) continue
      const idx = excelRows.findIndex((r, i) => {
        if (usedExcel.has(i)) return false
        const seq = r.sequence_no ?? r.track_number
        return seq != null && parseInt(seq, 10) === fmNum
      })
      if (idx >= 0) { pairs.push({ fm, row: excelRows[idx] }); usedExcel.add(idx) }
    }

    // Pass 2 — ISRC
    for (const fm of fmTracks) {
      if (pairs.find(p => p.fm.fm_record_id === fm.fm_record_id)) continue
      if (!fm.isrc) continue
      const idx = excelRows.findIndex((r, i) => {
        if (usedExcel.has(i)) return false
        return r.isrc && r.isrc.toUpperCase().trim() === fm.isrc.toUpperCase().trim()
      })
      if (idx >= 0) { pairs.push({ fm, row: excelRows[idx] }); usedExcel.add(idx) }
    }

    // Pass 3 — Levenshtein title
    for (const fm of fmTracks) {
      if (pairs.find(p => p.fm.fm_record_id === fm.fm_record_id)) continue
      // Derive a matchable title string from the FM audio file reference.
      // Applies titleFromFilename() to handle the standard Gallo convention
      // (ArtistName_SongTitle_CatalogNo.wav) — strips artist prefix and
      // catalogue suffix before fuzzy-comparing against the Excel title column.
      const base = fm.audio_container_url
        ? normTitle(titleFromFilename(fm.audio_container_url.split('\r')[0].replace('movie:', '').trim()))
        : normTitle(fm.title || '')
      if (!base) continue
      let bestIdx = -1, bestScore = 0
      excelRows.forEach((r, i) => {
        if (usedExcel.has(i)) return
        const score = fuzzyScore(base, normTitle(r.title))
        if (score > bestScore) { bestScore = score; bestIdx = i }
      })
      if (bestScore >= 0.72 && bestIdx >= 0) {
        pairs.push({ fm, row: excelRows[bestIdx] }); usedExcel.add(bestIdx)
      }
    }

    // Apply patches
    for (const { fm, row } of pairs) {
      if (row.sequence_no == null && row.track_number != null) row.sequence_no = row.track_number
      try {
        const { skipped } = await updateGalloWithDiagnosis(fm.fm_record_id, buildFieldDataBatch(row))
        catResult.matched++
        console.log(`[BatchEnrich] ✓ ${cat} — ${fm.fm_record_id} → ${row.title}${skipped.length ? ` (skipped: ${skipped.join(', ')})` : ''}`)
      } catch (e) {
        catResult.errors.push({ title: row.title, error: e.message })
        console.warn(`[BatchEnrich] ✗ ${cat} — ${fm.fm_record_id}: ${e.message}`)
      }
    }

    // Unmatched
    const pairedFmIds = new Set(pairs.map(p => p.fm.fm_record_id))
    catResult.unmatched_fm = fmTracks
      .filter(fm => !pairedFmIds.has(fm.fm_record_id))
      .map(fm => fm.title || fm.fm_record_id)
    catResult.unmatched_xl = excelRows
      .filter((_, i) => !usedExcel.has(i))
      .map(r => r.title || `row ${r._row}`)

    // Create Tape Files Master record for this catalogue
    if (catResult.matched > 0) {
      const firstRow = pairs[0]?.row
      if (firstRow) {
        createTapeFileRecord({
          album_artist: firstRow.album_artist || firstRow.artist_name,
          album:        firstRow.album_title,
          catalogue_no: firstRow.catalogue || cat,
        }).then(t  => console.log(`[BatchEnrich] Tape Files record created: ${t.tapeRecordId}`))
          .catch(e => console.warn(`[BatchEnrich] Tape Files create failed: ${e.message}`))
      }
    }
  ...
  const totalMatched = catalogueResults.reduce((s, r) => s + r.matched, 0)
  res.json({ ok: true, catalogues: catalogueResults.length, total_matched: totalMatched, results: catalogueResults })
})
