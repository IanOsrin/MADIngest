 ...
 */
import { visionStat, visionList } from './vision-drive.js'
import { normTitle } from './gallo-vision-link.js'
...
export function resolveGalloAudio(fieldData = {}) {
  ...
  const candidates = [
    ['Audio_URL', f['Audio_URL'] || f['audio_Url'] || f['Audio_Url'] || f['audio_url']],
...
/**
 * Resolve, then LOCATE the real file on Vision — self-healing against filename
 * drift. FileMaker often recorded a bare "<track>.wav" while the actual object
 * is "Miriam Makeba_<track>.wav" (artist-prefixed), so an exact-key lookup
 * 404s. On a miss we list the folder and match the record's Track Name to a
 * file by normalised name (tolerates the prefix, punctuation, case). Used by
 * the audio endpoint so playback works despite the drift.
 */
export async function resolveGalloAudioLive(fieldData = {}) {
  const r = resolveGalloAudio(fieldData)
  if (!r.ok || r.kind !== 'vision') return r
  try { if (await visionStat(r.path)) return { ...r, exists: true } } catch { /* fall through */ }

  const folder = r.path.replace(/\/[^/]+$/, '')
  try {
    const { entries } = await visionList(folder)
    const files = entries.filter(e => e.type === 'file')
    const want = normTitle((fieldData['Track Name'] || '') || r.filename)
    if (want) {
      let hit = files.find(f => normTitle(f.name) === want)
      if (!hit) hit = files.find(f => { const nf = normTitle(f.name); return nf.includes(want) || want.includes(nf) })
      if (hit) {
        const newPath = `${folder}/${hit.name}`
        return { ...r, path: newPath, key: newPath.replace(/^\/[^/]+\//, ''), filename: hit.name, exists: true, relocated: true }
      }
    }
