...
import { findMamTracksByCatalogue, updateMamRecord } from './fm-mam.js'
...
export const DB_LABELS = { gallo: 'Gallo Catalogue', cms2024: 'CMS 2024', streamer: 'MadStreamer', mam: 'Music Arena Master' }
...
// MAM splits albums and songs into separate tables, so it gets two builder
// tables: album-level keys write the Albums record, everything else the Songs
// record. applyDbSync routes by key; the merged table serves buildFieldData.
const MAM_ALBUM_BUILDERS = {
  album_title:  v => ({ 'Album Title': str(v) }),
  album_artist: v => ({ 'Album Artist': str(v) }),
  release_date: v => ({ 'Release Date': toFmDate(v) }),
  catalogue:    v => ({ 'Album Catalogue Number': str(v), 'Reference Catalogue Number': str(v) }),
...
const MAM_SONG_BUILDERS = {
  ...
  duration:              v => ({ 'Duration': toFmTime(v) }),
  genre:                 v => {
    // Genre verbatim (DDEX-facing); Local Genre only via the taxonomy —
    // MAM's Local Genre was normalised 2026-09-08 and must stay that way.
    const out = { 'Genre': str(v) }
    const lg = normalizeGenre(v)
    if (lg) out['Local Genre'] = lg
    return out
  },
  // Composers keeps the role tags as supplied; Composer … Composer 4 get one
  // plain name each (lib/credits.js).
  composer:              v => mamComposerFields(v),
  publisher:             v => ({ 'Publishers': str(v) }),
  producer:              v => ({ 'Producers': str(v), 'Producer': str(v) }),
  parental:              v => ({ 'Lyrical Content Rating': str(v) }),
  rights_territories:    v => ({ 'Rights Territories': str(v) }),
  featured_artist:       v => ({ 'Featured Artist': str(v) }),
}
const MAM_BUILDERS = { ...MAM_SONG_BUILDERS, ...MAM_ALBUM_BUILDERS }

/**
 * Which cache keys live on MAM's Songs layout and which on Albums.
 *
 * Gallo keeps album fields on every song row, so an album edit there is N
 * writes. MAM has a real Albums record, so the album tab has to route each
 * field to the right layout — one write for an album field, not one per track.
 */
export const MAM_SONG_KEYS  = Object.freeze(Object.keys(MAM_SONG_BUILDERS))
export const MAM_ALBUM_KEYS = Object.freeze(Object.keys(MAM_ALBUM_BUILDERS))

const BUILDERS = { gallo: GALLO_BUILDERS, cms2024: CMS_BUILDERS, streamer: STREAMER_BUILDERS, mam: MAM_BUILDERS }
...
/** Cache-row-shaped object → the metadata shape createGalloRecord expects.
 *  Exported for the Album tab's add-track flow, which builds the same shape. */
export function galloMetadataFromRow(r) {
  return {
    title:                 r.track_name,
...
// MAM matches carry Songs fieldData plus the album's fieldData for album keys.
function oldFromMam(t, key) {
  const f = t.fieldData || {}
  const a = t.albumFieldData || {}
  const pick = (src, ...names) => { for (const n of names) { const v = src[n]; if (v != null && String(v).trim() !== '') return v } return null }
  switch (key) {
    case 'album_title':           return pick(a, 'Album Title')
    case 'album_artist':          return pick(a, 'Album Artist')
    case 'release_date':          return pick(a, 'Release Date')
    case 'catalogue':             return pick(a, 'Album Catalogue Number', 'Reference Catalogue Number')
    case 'barcode':               return pick(a, 'UPC')
    case 'label':                 return pick(a, 'Label')
    case 'original_release_date': return pick(f, 'Original Release Date')
    case 'language':              return pick(f, 'Language Code', 'Language')
    case 'isrc':                  return t.isrc ?? null
    case 'p_line':                return pick(f, 'pLine')
    case 'c_line':                return pick(f, 'cLine')
    case 'track_name':            return t.title ?? null
    case 'track_artist':          return pick(f, 'Track Artist')
    case 'seq':                   return t.sequence_no ?? null
    case 'duration':              return pick(f, 'Duration')
    case 'genre':                 return pick(f, 'Genre') ?? pick(a, 'Genre')
    case 'composer':              return pick(f, 'Composers', 'Composer')
    case 'publisher':             return pick(f, 'Publishers')
    case 'producer':              return pick(f, 'Producers', 'Producer')
    case 'parental':              return pick(f, 'Lyrical Content Rating')
    case 'rights_territories':    return pick(f, 'Rights Territories')
    case 'featured_artist':       return pick(f, 'Featured Artist')
    default:                      return null
  }
}
...
export async function previewDbSync({ edits = [] }) {
  ...
  await Promise.all([...catalogues].map(async cat => {
    const [g, c, s, m] = await Promise.allSettled([
      findGalloRecordsByCatalogue(cat),
      findCmsRecordsByCatalogue(cat),
      findStreamerRecordsByCatalogue(cat, { includeFieldData: true }),
      findMamTracksByCatalogue(cat),
    ])
    // MAM tracks carry their album's recordId + fieldData so album-level keys
    // can diff and patch the Albums record.
    const mamTracks = m.status === 'fulfilled' && m.value
      ? m.value.tracks.map(t => ({ ...t, albumRecordId: m.value.album.recordId, albumFieldData: m.value.album.fieldData }))
      : (m.status === 'fulfilled' ? [] : null)
    perCat.set(cat, {
    ...
    for (const [db, r] of [['gallo', g], ['cms2024', c], ['streamer', s], ['mam', m]]) {
  ...
  for (const w of work) {
    ...
    const keys = [...w.keys]
    for (const db of ['gallo', 'cms2024', 'streamer', 'mam']) {
      ...
      const oldOf = db === 'streamer' ? oldFromStreamer : db === 'mam' ? oldFromMam : oldFromMapped
      if (track) takenFor(w.fetchCat, db).add(track)
      // MAM is update-only: creating there needs AlbumID allocation and the
      // Fill-MAM flow. An unmatched track is surfaced as a warning instead of
      // a create target the apply step can't honour.
      if (db === 'mam' && !track) {
        warnings.push(`"${w.row.track_name || '?'}" (${w.fetchCat}) not found in Music Arena Master — add the album via Fill MAM, then re-push`)
        continue
      }
      targets.push({
        db,
        ...
        albumRecordId: db === 'mam' && track ? track.albumRecordId : undefined,
...
export async function applyDbSync(targets = []) {
  ...
  const mamAlbumsPatched = new Set()   // Albums recordIds already written this run
  let albumRow = null
    ...
    try {
      if (t.action === 'update') {
        ...
        if (t.db === 'gallo') {
          ...
          // Songs and Albums are separate tables in MAM — split the edit by
          // key. The Albums record is shared by every track target of the
          // album, so it is patched once per apply run.
          const songKeys  = t.keys.filter(k => MAM_SONG_BUILDERS[k])
          const albumKeys = t.keys.filter(k => MAM_ALBUM_BUILDERS[k])
          const written = []
          if (songKeys.length) {
            const fd = {}
            for (const k of songKeys) Object.assign(fd, MAM_SONG_BUILDERS[k](t.row[k]))
            if (Object.keys(fd).length) { await updateMamRecord('Songs', t.recordId, fd); written.push(...songKeys) }
          }
          if (albumKeys.length && t.albumRecordId && !mamAlbumsPatched.has(t.albumRecordId)) {
            const fd = {}
            for (const k of albumKeys) Object.assign(fd, MAM_ALBUM_BUILDERS[k](t.row[k]))
            if (Object.keys(fd).length) {
              await updateMamRecord('Albums', t.albumRecordId, fd)
              mamAlbumsPatched.add(t.albumRecordId)
              written.push(...albumKeys)
            }
          }
          results.push({ ...ref, ok: true, fields: written })
        } else {
        ...
        if (t.db === 'mam') throw new Error('Creating records in Music Arena Master is not supported here — use Fill MAM')
