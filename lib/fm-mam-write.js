 ...
 *    joined to Songs by CATALOGUE NUMBER: Albums::Album Catalogue Number ==
 *    Songs::Album Catalogue (Ian moved the relationship off AlbumID onto the
 *    catalogue, 2026-09-01 — easier to work with by hand). So the field that
 *    actually binds a song to its album is `Album Catalogue`, and it must match
 *    the album's catalogue string EXACTLY — a spacing variant silently orphans
 *    the track. Both sides are written from one value here for that reason.
 *  - MasterID / RecordingID / AlbumID are NOT auto-enter (verified against the
 *    layout metadata) — this module has to allocate them. AlbumID is no longer
 *    the join, but every one of the 127k existing songs carries one, so new
 *    rows keep the convention rather than becoming the odd ones out. It reads the current
...
export async function mamSession() {
  ...
  const api = {
    ...
    async patch(layout, recordId, fieldData) {
      const body = clean(fieldData)
      if (!Object.keys(body).length) return false
      const res = await fetch(`${base()}/layouts/${encodeURIComponent(layout)}/records/${recordId}`,
        { method: 'PATCH', headers: H, body: JSON.stringify({ fieldData: body }) })
      const jj = await res.json()
      const code = jj?.messages?.[0]?.code
      if (code !== '0') throw new Error(`${jj?.messages?.[0]?.message || 'update failed'} (code ${code})`)
      return true
    },
...
/**
 * A free AlbumID for this catalogue. Truncation at 20 chars means two long
 * catalogues can derive the same one; MAM's own answer is a short suffix
 * (ALB-AMIFAKUGALLOREMIXEDF-E7E1), so this follows it rather than inventing a
 * scheme. Not fatal either way now that the join is the catalogue number — but
 * a duplicate ID is still confusing to read, and may trip field validation.
 */
export async function freeAlbumId(db, catalogue) {
  const base = albumIdFor(catalogue)
  if (!(await albumIdTaken(db, base))) return base
  const raw = String(catalogue || '')
  let h = 0
  for (let i = 0; i < raw.length; i++) h = (h * 31 + raw.charCodeAt(i)) >>> 0
  for (let n = 0; n < 16; n++) {
    const id = `${base}-${((h + n) & 0xffff).toString(16).toUpperCase().padStart(4, '0')}`
    if (!(await albumIdTaken(db, id))) return id
  }
  throw new Error(`could not find a free AlbumID for ${catalogue}`)
}
...
export async function createMamSong(db, m) {
  return db.create(SONGS_LAYOUT(), {
    ...
    'Language': m.language,
    'Language Code': m.language_code,
    // Composers keeps the role tags; Composer … Composer 4 one plain name each.
    ...mamComposerFields(Array.isArray(m.composers) ? m.composers.join('; ') : m.composers),
