...
export const DEFAULT_OUT_DIR = join(homedir(), 'Downloads', 'mad-youtube-pilot')

// ── environment support ──────────────────────────────────────────────────────
export async function checkRenderSupport() {
  const problems = []
  if (process.platform !== 'darwin') {
    problems.push('Video rendering needs macOS (text overlays use AppKit via osascript) — run the desktop GalloIngest, not the hosted one.')
  }
  try {
...
async function renderShort(t, art, audio, out, { shortLen, shortOffset }) {
  ...
  await ffmpeg([
    '-y', '-ss', String(shortOffset), '-i', audio, '-loop', '1', '-i', art, '-i', overlay,
    '-filter_complex', filter, '-map', '[vout]', '-map', '0:a',
