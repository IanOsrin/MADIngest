 ...
 *   filterTail — filter step consuming [base] and producing [still]; the PNG,
 *                when there is one, is input 1 (after the sleeve art)
...
async function buildOverlay(spec) {
  if (process.platform === 'darwin' && !(await hasDrawtext())) {
    ...
    return { inputs: ['-i', spec.pngOut], filterTail: '[base][1:v]overlay=0:0[still]', cleanup: () => {} }
  }
  ...
  return {
    inputs: [],
    filterTail: `[base]${steps.join(',')}[still]`,
...
// Hashtags: YouTube shows the first three above the title. Keep them short —
// one evergreen, one genre, one artist (stripped to CamelCase-ish).
function hashtags(t) {
  const tag = s => s && '#' + String(s).replace(/['’]/g, '').split(/[^a-zA-Z0-9]+/)
    .filter(Boolean).map(w => w[0].toUpperCase() + w.slice(1)).join('')
  return ['#SouthAfricanMusic', tag(t.genre), tag(t.artist)].filter(Boolean).slice(0, 3).join(' ')
}
...
function sidecar(t, file, kind) {
  ...
  // utm_campaign groups every video under one line in Umami.
  const shareUrl = `${SITE}/?t=${t.recordId}&utm_source=youtube&utm_medium=video&utm_content=${kind}&utm_campaign=tasters`
  const lines = [
    `SUGGESTED TITLE:`,
    ...
    ``,
    // Provenance — the one thing bootleg rips and Topic uploads can't say.
    `Official upload from the Gallo Record Company vault — home of 100 000+ South African classics, streaming at ${SITE}`,
    ``,
    t.pLine || '',
    ``,
    hashtags(t),
...
// ── renderers ────────────────────────────────────────────────────────────────
// Two passes. Everything that never changes — the blurred background, the
// sleeve and the text — is drawn ONCE into a still PNG; the video encode then
// only overlays the moving waveform on that picture. Drawing the blur and text
// for every frame made a full-length art-track take well over ten minutes on
// the hosted instance's single CPU (2026-09-17).
async function renderStill({ art, overlay, width, height, sigma, sleeveSize, sleeveX, sleeveY, out }) {
  const filter = [
    `[0:v]scale=${width}:${height}:force_original_aspect_ratio=increase,crop=${width}:${height},gblur=sigma=${sigma},eq=brightness=-0.25[bg]`,
    `[0:v]scale=${sleeveSize}:${sleeveSize}[sleeve]`,
    `[bg][sleeve]overlay=${sleeveX}:${sleeveY}[base]`,
    overlay.filterTail
  ].join(';')
  await ffmpeg(['-y', '-i', art, ...overlay.inputs, '-filter_complex', filter, '-map', '[still]', '-frames:v', '1', out])
  return out
}

async function encodeWithWaves({ audio, still, waves, audioArgs = [], outputArgs = [], out }) {
  await ffmpeg([
    // The still is decoded once and repeated by the loop filter; looping the PNG
    // as an input decoded it again for every frame, doubling the encode time.
    '-y', ...audioArgs, '-i', audio, '-i', still,
    '-filter_complex', `[1:v]format=yuv420p,loop=loop=-1:size=1:start=0,fps=25[bg];` +
      `[0:a]showwaves=s=${waves.w}x${waves.h}:mode=cline:rate=25:colors=white@0.45[waves];` +
      `[bg][waves]overlay=${waves.x}:${waves.y}:shortest=1[vout]`,
    '-map', '[vout]', '-map', '0:a', ...outputArgs, '-shortest',
...
async function renderArtTrack(t, art, audio, out, { excerpt }) {
  ...
  const overlay = await buildOverlay({
    ...
    items: [
      ...
      // Watermark → call to action: the description link is invisible while the
      // video plays; this corner is the one placement every viewer sees all song.
  ...
  const still = out.replace(/\.mp4$/, '.still.png')
  try {
    // ambient-blur background from the sleeve (never stretched art — blurred
    // fill), sharp sleeve left, text right
    await renderStill({ art, overlay, width: 1920, height: 1080, sigma: 40, sleeveSize: 760, sleeveX: 140, sleeveY: 160, out: still })
    // waveform strip along the bottom
    await encodeWithWaves({ audio, still, waves: { w: 1640, h: 110, x: 140, y: 930 }, outputArgs: durationArgs, out })
  } finally {
    overlay.cleanup()
    try { unlinkSync(still) } catch {}
  }
}
...
async function renderShort(t, art, audio, out, { shortLen, shortOffset }) {
  ...
  const still = out.replace(/\.mp4$/, '.still.png')
  try {
    await renderStill({ art, overlay, width: 1080, height: 1920, sigma: 45, sleeveSize: 880, sleeveX: 100, sleeveY: 290, out: still })
    await encodeWithWaves({
      audio, still, waves: { w: 880, h: 100, x: 100, y: 1560 },
      audioArgs: ['-ss', String(shortOffset)],
      outputArgs: ['-t', String(shortLen), '-af', `afade=t=in:d=0.8,afade=t=out:st=${shortLen - 1}:d=1`],
      out
    })
  } finally {
    overlay.cleanup()
