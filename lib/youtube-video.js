...
async function encodeWithWaves({ audio, still, waves, audioArgs = [], outputArgs = [], out }) {
  await ffmpeg([
    '-y', ...audioArgs, '-i', audio, '-loop', '1', '-framerate', '25', '-i', still,
    '-filter_complex', `[0:a]showwaves=s=${waves.w}x${waves.h}:mode=cline:rate=25:colors=white@0.45[waves];[1:v][waves]overlay=${waves.x}:${waves.y}[vout]`,
