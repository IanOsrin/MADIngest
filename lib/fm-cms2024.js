 ...
 *
 * `options.timeoutMs` overrides FM_TIMEOUT_MS for this call. Callers that put a
 * user behind the request want a much smaller budget than 60s, and this ABORTS
 * the request rather than abandoning it — an abandoned FM request still holds
 * its connection, which is what the serial queue in routes/ingest.js exists to
 * stop happening.
 */
...
export async function getLayoutFields(layoutName = LAYOUT, { timeoutMs } = {}) {
  if (_layoutFieldCache.has(layoutName)) return _layoutFieldCache.get(layoutName)

  const res  = await cmsFetch(`/layouts/${encodeURIComponent(layoutName)}`, { timeoutMs })
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
...
export async function searchRecords(term, opts = {}) {
  const { layout = LAYOUT, limit = 50, offset = 0, timeoutMs } = opts
  ...
  const known = await getLayoutFields(layout, { timeoutMs })
  const candidates = [
  ...
  const queries = candidates
    .filter(f => known.has(f))
    .flatMap(f => CAT_FIELDS.has(f)
      ? catSearchVariants(t).map(v => ({ [f]: `*${v}*` }))
      : [{ [f]: `*${t}*` }])
  if (!queries.length) return { tracks: [], foundCount: 0 }

  const { records, foundCount } = await findRecords(queries, { layout, limit, offset, timeoutMs })
