...
/**
 * MD5 of a whole Vision object, computed on the stream (nothing buffered in
 * RAM). Uppercase hex — the convention AudioHashSum / the DDEX build use.
 * Costs a full read of the file (~seconds per master); call it only where the
 * hash is actually stored.
 */
export async function computeVisionMd5(path) {
  const { createHash } = await import('node:crypto')
  const obj = await visionOpen(path)
  const hash = createHash('md5')
  const stream = obj.Body.transformToWebStream ? obj.Body.transformToWebStream() : obj.Body
