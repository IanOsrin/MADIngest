...
const _sweep = { running: false, queue: [], lastRunAt: null, lastFound: 0, lastIndexed: 0, lastError: null }
...
export async function sweepNewAlbums({
  ...
  try {
    ...
    while (_sweep.queue.length && calls < maxCalls && Date.now() - t0 < budgetMs) {
      ...
      let entries
      try { entries = (await visionList(folder)).entries || []; calls++ }
      catch { continue }                        // a folder that vanished mid-walk — skip
    ...
    for (const leaf of fresh) {
      ...
      try {
        const r = await reindexVisionPath(leaf, { cacheFile })
    ...
    if (found) console.log(`[vision-detect] ${found} new album folder(s), ${indexed} indexed · ${calls} listings in ${Math.round((Date.now() - t0) / 1000)}s · ${_sweep.queue.length} queued`)
