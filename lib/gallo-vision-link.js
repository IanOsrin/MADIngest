...
// ── New-album detector ───────────────────────────────────────────────────────
// An album dropped onto Vision by Cyberduck (i.e. NOT through the app, so no
// upload hook fires) is invisible to Add Album until the parent's turn comes
// round — and the album-bearing parents are the big, once-a-day roots, so that
// wait can be a day. `Falabo_Intukuthelo_CDIZI 117` was uploaded at 06:31 and
// still unfindable hours later for exactly this reason (Ian, 2026-09-04).
//
// The full-relist that would catch it is minutes long (44k objects under one
// WAV root), which is why it is throttled. But a DELIMITER listing — one folder's
// immediate children, no object walk — costs ~150ms, so we can walk the folder
// TREE of a few roots cheaply and splice in any album folder the index does not
// already hold. Container folders (labels, artists) are listed every sweep —
// that is how a new child is spotted — while album leaves already in the index
// are skipped, so a settled estate costs only its container count per sweep
// (~200 listings / ~30s for Licensed WAVs_Local). A call budget caps one sweep
// and a persisted queue lets the next continue, so no single sweep runs long.
const _sweep = { running: false, startedAt: 0, queue: [], lastRunAt: null, lastFound: 0, lastIndexed: 0, lastError: null }

// Reject a promise that takes too long instead of awaiting it forever. The
// shared S3 client has no socket timeout configured, so one stalled listing
// would otherwise hang the whole walk and leave _sweep.running stuck true,
// silently deadlocking every later tick (observed live 2026-09-04). A delimiter
// listing is normally ~150ms; 20s is a stall, not a slow response.
const withTimeout = (p, ms, label) => Promise.race([
  p,
  new Promise((_, rej) => setTimeout(() => rej(new Error(`timeout after ${ms}ms: ${label}`)), ms).unref?.()),
])

// Roots whose subtrees are walked for new albums. Comma-separated browse paths;
// override with VISION_DETECT_ROOTS. Defaults to the album-bearing WAV roots and
// Rendered Files — the places albums actually land — NOT the whole estate.
function detectRoots() {
  const raw = process.env.VISION_DETECT_ROOTS ||
    '/gallo-music-files-wavs/Gallo Record Company Music - WAVs/Licensed WAVs,' +
    '/gallo-music-files-wavs/Gallo Record Company Music - WAVs/Owned WAVs,' +
    '/gallo-music-files-wavs/Gallo Record Company Music - WAVs/Owned WAVs_1,' +
    '/gallo-music-files-wavs/Gallo Record Company Music - WAVs/JV Wavs,' +
    '/gallo-digital-cupboard/Rendered Files'
  return raw.split(',').map(s => s.trim()).filter(Boolean)
}

export function sweepStatus() { return { ..._sweep, queued: _sweep.queue.length } }

/**
 * Walk the detect-roots' folder tree, indexing any album folder the index does
 * not already know. Returns what it found. Cheap and interruptible: bounded by
 * a call budget and a wall-clock budget, resuming from a persisted queue.
 */
export async function sweepNewAlbums({
  cacheFile,
  budgetMs = Number(process.env.VISION_DETECT_BUDGET_MS || 90000),
  maxCalls = Number(process.env.VISION_DETECT_MAX_CALLS || 500),
} = {}) {
  const listMs = Number(process.env.VISION_DETECT_LIST_TIMEOUT_MS || 20000)
  // Self-healing lock: a sweep that hung (see withTimeout) would leave
  // _sweep.running stuck true forever. If the flag is set but the run is older
  // than two budgets, treat the previous run as dead and take over.
  if (_sweep.running && Date.now() - _sweep.startedAt > Math.max(budgetMs * 2, 5 * 60 * 1000)) {
    console.warn(`[vision-detect] clearing a stale sweep lock (started ${Math.round((Date.now() - _sweep.startedAt) / 1000)}s ago)`)
    _sweep.running = false
  }
  // Never fight a full refresh or another sweep for the drive or the index file.
  if (_state.running || _sweep.running) return { started: false, reason: 'busy' }
  _sweep.running = true
  _sweep.startedAt = Date.now()
  const t0 = Date.now()
  let calls = 0, found = 0, indexed = 0
  try {
    // Already-searchable album folders = the parent dir of every indexed audio
    // file. Same path form the walk builds, so membership tests line up.
    const index = await loadVisionIndex({ cacheFile })
    const known = new Set()
    if (index) for (const f of (index.files || [])) known.add(f.path.replace(/\/[^/]+$/, ''))

    if (!_sweep.queue.length) _sweep.queue = detectRoots()   // new rotation

    const fresh = []
    while (_sweep.queue.length && calls < maxCalls && Date.now() - t0 < budgetMs) {
      const folder = _sweep.queue.shift()
      if (known.has(folder)) continue          // an indexed album leaf — nothing new below it
      let entries
      try { entries = (await withTimeout(visionList(folder), listMs, folder)).entries || []; calls++ }
      catch (e) { _sweep.lastError = e.message; continue }  // vanished or stalled mid-walk — skip this folder
      const hasAudio = entries.some(e => (e.type === 'file' || e.isDir === false) && AUDIO_RE.test(e.name || ''))
      if (hasAudio && !known.has(folder)) { fresh.push(folder); found++ }
      // Descend into subfolders whether or not this folder held audio: some
      // labels keep loose files beside their album subfolders. Album leaves are
      // the common no-subdir case and cost nothing to have queued.
      for (const e of entries) {
        if (e.type === 'dir' || e.isDir) _sweep.queue.push(folder.replace(/\/+$/, '') + '/' + e.name)
      }
    }

    // Splice each new album in on its own — seconds each, and it releases the
    // index lock between folders so a refresh is never blocked for long.
    for (const leaf of fresh) {
      if (_state.running) { _sweep.queue.unshift(leaf); break }  // refresh started — leave it for next sweep
      try {
        const r = await withTimeout(reindexVisionPath(leaf, { cacheFile }), listMs * 3, `reindex ${leaf}`)
        if (r.ok && r.audio) { indexed++; known.add(leaf) }
      } catch (e) { _sweep.lastError = e.message }
    }

    _sweep.lastRunAt = Date.now(); _sweep.lastFound = found; _sweep.lastIndexed = indexed
    // Log every completed sweep (not only ones that found something), so a run
    // is always visible in the log — a silent detector is impossible to trust.
    console.log(`[vision-detect] sweep done · ${found} new, ${indexed} indexed · ${calls} listings in ${Math.round((Date.now() - t0) / 1000)}s · ${_sweep.queue.length} queued`)
    return { started: true, found, indexed, calls, queued: _sweep.queue.length }
  } catch (e) {
    _sweep.lastError = e.message
    console.error('[vision-detect] sweep failed:', e.message)
    return { started: true, error: e.message, found, indexed }
  } finally {
    _sweep.running = false
  }
}

/** Reindex exactly the folder an in-app upload just wrote to. Fire-and-forget. */
export function reindexAfterUpload(folder, { cacheFile } = {}) {
  if (!folder) return
  reindexVisionPath(folder, { cacheFile })
    .then(r => r?.ok
      ? console.log(`[vision-detect] post-upload reindex ${folder}: ${r.audio} audio`)
      : console.warn(`[vision-detect] post-upload reindex ${folder} deferred: ${r?.reason || 'unknown'}`))
    .catch(e => console.warn(`[vision-detect] post-upload reindex ${folder} failed: ${e.message}`))
}

/**
 * Run the new-album sweep on its own cadence — shorter than the heavy refresh,
 * since it is cheap — so a Cyberduck upload surfaces within a sweep or two.
 */
export function startVisionNewAlbumSweep({ cacheFile } = {}) {
  const every = Number(process.env.VISION_DETECT_MS || 5 * 60 * 1000)
  if (!every || every < 0) return null            // set to 0 to switch off
  const tick = () => {
    if (_state.running || _sweep.running) return
    sweepNewAlbums({ cacheFile }).catch(() => { /* logged in sweep */ })
  }
  setTimeout(tick, 90 * 1000).unref?.()           // once, after the first refresh has had a head start
  const t = setInterval(tick, every)
  t.unref?.()
  console.log(`[vision-detect] new-album sweep every ${Math.round(every / 60000)} min · roots: ${detectRoots().length}`)
  return t
}
...
export function matchTracksToFiles(tracks, files) {
  ...
  // lone space defeats a literal substring test, so a track differing from its
  // file only in hyphenation/spacing went unmatched. Same fix as the vision-
  // import matcher (Ian, 2026-09-04, CDIZI 130 "iSurprise"). The 3-char floor
  // stops a tiny title matching everything.
  const squash = (s) => s.replace(/[^a-z0-9]/g, '')
  // Drop a leading track number ("01 Beautiful Me" -> "Beautiful Me") for title
  // comparison, so a plain-numbered file matches exactly instead of falling to
  // containment — where a short title that is a substring of a longer track's
  // name grabs the longer file under the longest-first tiebreak (BCCDJV 005
  // "Beautiful Me" stole track 5's file; Ian, 2026-09-11). Same fix as the
  // vision-import matcher. Only 1–2 digits + a separator + a title are peeled.
  const fileNorm = (f) => normTitle(String(f.name).replace(/^\s*\d{1,2}[\s._-]+(?=\S)/, ''))

  for (const t of tracks) {
    const nt = normTitle(t.title)
    if (!nt) { tracksNoAudio.push(t); continue }
    // exact normalised match first, then a space-insensitive contains either-way
    // (handles a trailing "(Radio Edit)", a leading track number, or a hyphen/
    // spacing gap). Longest filename first so the most specific candidate wins
    // when the album name recurs across every file in the folder.
    let hitIdx = null
    for (const [i, f] of remaining) if (fileNorm(f) === nt) { hitIdx = i; break }
    if (hitIdx == null) {
      const sw = squash(nt)
      if (sw.length >= 3) {
        let best = null
        for (const [i, f] of remaining) {
          const nf = squash(fileNorm(f))
          if (nf.includes(sw) || sw.includes(nf)) {
            const len = fileNorm(f).length
            if (!best || len > best.len) best = { i, len }
          }
        }
        if (best) hitIdx = best.i
      }
    }
