...
export async function createHeroBanner(b = {}) {
  ...
  if (b.startDate) fieldData.Start_Date = String(b.startDate).trim()
  if (b.endDate)   fieldData.End_Date   = String(b.endDate).trim()
...
export async function updateHeroBanner(recordId, patch = {}) {
  ...
  for (const [k, fmField] of Object.entries(allowed)) {
    ...
    fieldData[fmField] = k === 'active' ? (patch[k] ? '1' : '0') : String(patch[k])
  }
