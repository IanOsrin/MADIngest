...
/** Flag (or clear) a record's audio as bad. Stores the flag date so the
 *  repair worklist shows when each problem was caught. Requires the
 *  `Bad_Audio` text field to be ON the API_Album_Songs layout. */
export async function setBadAudio(recordId, flag) {
  ...
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_LAYOUT)}/records/${encodeURIComponent(recordId)}`,
    {
      method:  'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ fieldData: { 'Bad_Audio': value } })
    }
  )
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
    const code = json?.messages?.[0]?.code
    const hint = code === '102' ? ' — is the Bad_Audio field on the API_Album_Songs layout?' : ''
    throw new Error(`Bad_Audio update failed (record ${recordId}): ${json?.messages?.[0]?.message || `HTTP ${res.status}`}${hint}`)
  }
  return value
}

/** Every record carrying a Bad_Audio flag — the audio-repair worklist. */
export async function findBadAudioSongs({ maxRecords = 5000, pageSize = 500 } = {}) {
  const out = []
  let offset = 1
  while (out.length < maxRecords) {
    const res = await msFetch(
      `/layouts/${encodeURIComponent(MADSTREAMER_FM_LAYOUT)}/_find`,
      {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ query: [{ 'Bad_Audio': '*' }], limit: pageSize, offset })
      }
    )
    const json = await res.json().catch(() => ({}))
    if (!res.ok) {
      ...
      const code = json?.messages?.[0]?.code
      const hint = code === '102' ? ' — is the Bad_Audio field on the API_Album_Songs layout?' : ''
      throw new Error(`Bad_Audio find failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}${hint}`)
    }
    const page = json?.response?.data || []
    for (const r of page) {
      const f = r.fieldData || {}
      out.push({
        ...
        year:      String(f['Year of Release'] || '').trim() || null,
        s3url:     String(f['S3_URL'] || '').trim() || null,
        flagged:   String(f['Bad_Audio'] || '').trim(),
      })
    }
    if (page.length < pageSize) break
    offset += page.length
