 ...
 * place. S3 keys are stable per playlist slug (artwork/playlist-<slug>.jpg), so
 * a re-upload overwrites cleanly, and an orphaned object costs pennies whereas
 * a wrongly-deleted cover costs a re-make. The usual reason for deleting is a
 * record created under a mistyped playlist name, where the image itself is
 * fine and about to be re-uploaded under the right one.
 */
...
/** Re-tag one record's Local Genre. */
export async function setLocalGenre(recordId, genre) {
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_LAYOUT)}/records/${encodeURIComponent(recordId)}`,
    {
      method:  'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ fieldData: { 'Local Genre': genre } })
    }
  )
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
    throw new Error(`Local Genre update failed (record ${recordId}): ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }
}
