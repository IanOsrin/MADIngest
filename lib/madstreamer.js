 ...
 */
/**
 * The Artwork record that owns a GMVi, or null. Lets a file named
 * "GMVi5595.jpg" be traced back to its album, so a bulk cover upload can go
 * through lib/album-cover.js like every other cover.
 */
export async function findArtworkByGmvi(gmvi) {
  const g = String(gmvi || '').trim()
  if (!g) return null
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_ARTWORK_LAYOUT)}/_find`,
    { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: [{ [MADSTREAMER_FM_GMVI_FIELD]: `==${g}` }], limit: 5 }) }
  )
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
    if (json?.messages?.[0]?.code === '401') return null
  ...
  return {
    recordId:  String(rows[0].recordId),
    gmvi:      String(f[MADSTREAMER_FM_GMVI_FIELD] || '').trim() || g,
    catalogue: String(f[MADSTREAMER_FM_CATALOGUE_FIELD] || f[MADSTREAMER_FM_SINGLE_CATALOGUE_FIELD] || '').trim() || null,
    duplicates: rows.length > 1 ? rows.map(r => String(r.recordId)) : null,
  }
}
...
/**
 * Patch the album's Tape Files Master record, keeping only fields that exist on
 * that layout. Covers are refused here as everywhere: Artwork_S3_URL goes
 * through lib/album-cover.js, never a general-purpose patch.
 */
export async function updateTapeFileRecord(recordId, fieldData) {
  if (!recordId) throw new Error('updateTapeFileRecord: recordId required')
  if ('Artwork_S3_URL' in (fieldData || {})) assertPipelineCoverUrl(fieldData.Artwork_S3_URL, 'Tape Files Artwork_S3_URL')
  const known = await getLayoutFields(MADSTREAMER_FM_TAPE_LAYOUT)
  const { kept, dropped } = filterToKnownFields(fieldData, known)
  if (!Object.keys(kept).length) return { recordId: String(recordId), written: [], dropped }
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_TAPE_LAYOUT)}/records/${recordId}`,
    { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fieldData: kept }) })
  if (!res.ok) {
    const json = await res.json().catch(() => ({}))
    throw new Error(`MadStreamer Tape Files update failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }
  return { recordId: String(recordId), written: Object.keys(kept), dropped }
}
...
/**
 * Point a catalogue's Tape Files Master record at a new cover URL.
 *
 * This is what the app actually reads — songs show `Tape Files::Artwork_S3_URL`
 * and the site rewrites it to artwork/resized/<name>_300.webp. Uploading a
 * replacement cover to S3 without this write leaves the record pointing at the
 * old master, so the new image is never requested by anybody.
 *
 * Returns { ok, recordId, previousUrl } — or { ok: false, reason } when there
 * is no Tape Files record for the catalogue, which the caller must surface
 * rather than treat as success: the cover is on S3 but nothing points at it.
 */
export async function setTapeFileArtworkUrl(catalogueNo, url) {
  const cat = String(catalogueNo || '').trim()
  if (!cat) throw new Error('catalogue number required')
  if (!/^https?:\/\//i.test(String(url || ''))) throw new Error(`refusing to write a non-URL artwork value: ${JSON.stringify(url)}`)
  // Only pipeline covers (artwork/GMVi<n>.jpg) may reach the website's album record.
  assertPipelineCoverUrl(url, 'MADStreamer Tape Files Artwork_S3_URL')

  const existing = await findTapeFileByCatalogue(cat)
  if (!existing) return { ok: false, reason: `no Tape Files Master record for catalogue "${cat}"` }
  ...
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_TAPE_LAYOUT)}/records/${existing.recordId}`,
    {
      method:  'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ fieldData: { 'Artwork_S3_URL': url } })
    }
  )
  const json = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(`Artwork URL update failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  return {
    ok: true,
    recordId: existing.recordId,
    previousUrl: String(existing.fieldData?.['Artwork_S3_URL'] || '') || null,
 ...
 */
// The rail a playlist appears on. 'Curated' (2026-09-10) is for guest
// selectors — the playlist is NAMED for the person (Nicky B, The Wizard) and
// the rail heading supplies the "Curated by". A category with no matching rail
// on the website renders nowhere, so these must be added in pairs.
export const PLAYLIST_CATEGORIES = Object.freeze(['Scene', 'Artist', 'Decade', 'Theme', 'Curated'])
 ...
 */
/**
 * Delete one Artist Bio record by FileMaker recordId. Permanent — FileMaker has
 * no undo — so the Biography tab asks for confirmation, naming the artist,
 * before it calls this.
 */
export async function deleteArtistBio(recordId) {
  const id = String(recordId || '').trim()
  if (!/^\d+$/.test(id)) throw new Error('A numeric recordId is required')
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_BIO_LAYOUT)}/records/${encodeURIComponent(id)}`,
    { method: 'DELETE' }
  )
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
    throw new Error(`Artist Bio delete failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }
 ...
 */
export async function findRecordsByCatalogue(catalogueNo, { includeFieldData = false } = {}) {
...
/**
 * Update one track record by FM recordId with raw field names. Filters the
 * payload against the live layout first (FM rejects the whole PATCH if any
 * field name is unknown). Used by the Cache Viewer's push-to-databases flow.
 * Returns { recordId, dropped }.
 */
export async function updateStreamerRecord(recordId, fieldData) {
  if (!recordId) throw new Error('updateStreamerRecord: recordId required')
  const known = await getLayoutFields(MADSTREAMER_FM_LAYOUT)
  const { kept, dropped } = filterToKnownFields(fieldData, known)
  if (!Object.keys(kept).length) {
    throw new Error(`No matching fields on MadStreamer ${MADSTREAMER_FM_LAYOUT} — none of [${Object.keys(fieldData).join(', ')}] exist on the layout`)
  }
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_LAYOUT)}/records/${recordId}`,
    { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fieldData: kept }) }
  )
  if (!res.ok) {
    const json = await res.json().catch(() => ({}))
    throw new Error(`MadStreamer update failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }
  return { recordId: String(recordId), dropped }
}
 ...
 */
export async function searchMadStreamerRecords(term, { limit = 50, timeoutMs } = {}) {
  if (!term || term.trim().length < 2) return { tracks: [], foundCount: 0 }
  const t = term.trim()

  const known = await getLayoutFields(MADSTREAMER_FM_LAYOUT, { timeoutMs })
  const candidates = [
    'Track Name', 'Track Artist', 'Album Artist', 'Album Title',
    'ISRC', 'Album Catalogue Number', 'Reference Catalogue Number', 'Filename',
  ]
  const CAT_FIELDS = new Set(['Album Catalogue Number', 'Reference Catalogue Number'])
  const queries = candidates.filter(f => known.has(f)).flatMap(f => CAT_FIELDS.has(f)
  ...
  const tracks = (json?.response?.data || []).map(r => {
    ...
    return {
      ...
      isrc:         f['ISRC']          || null,
      sequence_no:  isNaN(seq) ? null : seq,
      // Carried for the data-health checks, not for display. A song needs an
      // ISRC, a UPC and a cover or the site never renders it, and that rule
      // currently hides one record in five — so a search has to be able to say
      // "this track exists here but nobody can see it on the site".
      upc:          f['UPC'] || null,
      artwork_url:  f['Tape Files::Artwork_S3_URL'] || f['Artwork_S3_URL'] || null,
      audio_url:    f['S3_URL'] || null,
    }
  })
...
function mapPlaylistSong(r) {
  ...
  return {
    ...
    // Position within the PUBLIC PLAYLIST, distinct from sequence_no (the track's
    // place on its own album). The website already sorts playlist tracks by this
    // when any track carries it, so the running order set here is what listeners get.
    playlist_order: (() => {
      const raw = f['PublicPlaylistOrder']
...
/**
 * Songs for the Playlists tab, matched on artist OR track title.
 *
 * Separate query objects are OR'd by FileMaker, so one term sweeps Track
 * Artist, Album Artist and Track Name in a single find. Title matters as much
 * as artist when building a playlist — you often know the song and not who cut
 * it — and the in-playlist "Add songs" box has always offered title search in
 * its placeholder while this only ever looked at the two artist fields.
 *
 * `scope` narrows it when a common word floods the results: 'artist' or
 * 'title'; anything else (the default) searches both.
 */
export async function findStreamerSongs(term, { limit = 500, scope = 'any' } = {}) {
  if (!term || term.trim().length < 2) return []
  const t = term.trim()
  const query = []
  if (scope !== 'title') {
    query.push({ 'Track Artist': `*${t}*` }, { 'Album Artist': `*${t}*` })
  }
  if (scope !== 'artist') {
    query.push({ 'Track Name': `*${t}*` })
  }
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_LAYOUT)}/_find`,
    {
  ...
  if (!res.ok) {
    ...
    throw new Error(`MadStreamer song search failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }
  return sortByAlbumSeq((json?.response?.data || []).map(mapPlaylistSong))
}

/** @deprecated kept so nothing breaks on the old name — searches artists only. */
export const findStreamerSongsByArtist = (term, opts = {}) =>
  findStreamerSongs(term, { ...opts, scope: 'artist' })

/**
 * Write the running order for one track of a public playlist.
 *
 * Clearing is deliberate: passing null empties the field, so a playlist can be
 * put back to album order without deleting and rebuilding it.
 */
export async function setPublicPlaylistOrder(recordId, order) {
  const value = (order === null || order === undefined || order === '') ? '' : String(order)
  const res = await msFetch(
    `/layouts/${encodeURIComponent(MADSTREAMER_FM_LAYOUT)}/records/${encodeURIComponent(recordId)}`,
    { method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ fieldData: { PublicPlaylistOrder: value } }) }
  )
  const json = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(`Playlist order update failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  return { ok: true }
}
...
export async function findSongsByPlaylist(name) {
  ...
  const songs = (json?.response?.data || []).map(mapPlaylistSong)
  // A playlist that has been given a running order is shown in THAT order —
  // album order is only the fallback for one nobody has sequenced yet. Showing
  // a sequenced playlist in album order would make the drag handles lie about
  // what listeners hear.
  return songs.some(s => s.playlist_order !== null)
    ? songs.slice().sort((a, b) =>
        (a.playlist_order ?? Number.MAX_SAFE_INTEGER) - (b.playlist_order ?? Number.MAX_SAFE_INTEGER) ||
        String(a.title || '').localeCompare(String(b.title || '')))
    : sortByAlbumSeq(songs)
}
...
/**
 * Every stream event whose last-event timestamp falls in [from, to], both
 * FileMaker timestamps ("MM/DD/YYYY HH:MM:SS", UTC). Paged, because a find
 * returns at most a page at a time; the layout holds one record per listen
 * (~10k in the first nine months), so a whole year is a handful of requests.
 * Read-only — the website owns these records.
 */
export async function fetchStreamEvents({ from, to, pageSize = 1000, maxRecords = 200000 } = {}) {
  const layout = process.env.MADSTREAMER_FM_STREAM_LAYOUT || 'API_Stream_Events'
  const out = []
  for (let offset = 1; out.length < maxRecords; offset += pageSize) {
    const res = await msFetch(`/layouts/${encodeURIComponent(layout)}/_find`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        query: [{ TimestampUTC: `${from}...${to}` }],
        limit: pageSize,
        offset,
        sort: [{ fieldName: 'TimestampUTC', sortOrder: 'ascend' }],
      })
    })
    const json = await res.json().catch(() => ({}))
    const code = json?.messages?.[0]?.code
    if (code === '401') break                       // FileMaker's "no records match"
    if (!res.ok) {
      throw new Error(`MadStreamer stream events find failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
    }
    const page = json?.response?.data || []
    out.push(...page.map(d => d.fieldData))
...
export async function getLayoutFields(layoutName = MADSTREAMER_FM_LAYOUT, { timeoutMs } = {}) {
  if (_layoutFieldCache.has(layoutName)) return _layoutFieldCache.get(layoutName)

  const res = await msFetch(`/layouts/${encodeURIComponent(layoutName)}`, { timeoutMs })
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
 ...
 *  Pass '(blank)' (or '') to find tracks with an EMPTY Local Genre — FM's
 *  empty-field find operator is a bare '='. */
export async function findSongsByLocalGenre(genre, { maxRecords = 15000, pageSize = 1000 } = {}) {
  const wantBlank = !genre || String(genre).trim() === '' || String(genre).trim().toLowerCase() === '(blank)'
  const out = []
  let offset = 1
  while (out.length < maxRecords) {
    const res = await msFetch(
      `/layouts/${encodeURIComponent(MADSTREAMER_FM_LAYOUT)}/_find`,
      {
        ...
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({
          query: [{ 'Local Genre': wantBlank ? '=' : `==${genre}` }],
          limit: pageSize,
          offset,
        })
      }
