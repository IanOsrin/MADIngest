...
// MAM files this catalogue's albums under the vinyl/CD spelling inconsistently:
// the site and CMS say CDGURB 033, MAM says GURB 033. Spacing variants alone
// missed that and built a duplicate album (2026-09-18). A prefix variant is
// NEVER adopted silently, though — CDGMP 1823 and GMP 1823 are different
// albums — so a candidate is offered and the operator decides.
const FORMAT_PREFIXES = ['CD', 'MC', 'LP', 'CDL', 'MCL', 'DVD']
export function catalogueVariants(cat) {
  const c = S(cat)
  const out = new Set([c, c.replace(/\s+/g, ''), c.replace(/^([A-Za-z]+)\s*(\d)/, '$1 $2')])
  const m = c.match(/^([A-Za-z]+)\s*(.*)$/)
  if (m) {
    const [, letters, rest] = m
    const up = letters.toUpperCase()
    const carried = FORMAT_PREFIXES.find(p => up.startsWith(p) && letters.length > p.length)
    if (carried) {
      // CDGURB 033 → GURB 033: drop the format prefix it already carries.
      const bare = letters.slice(carried.length)
      out.add(`${bare} ${rest}`.trim()); out.add(`${bare}${rest}`.trim())
    } else {
      // GURB 033 → CDGURB 033 / MCGURB 033: the same release on another format.
      for (const p of FORMAT_PREFIXES) { out.add(`${p}${letters} ${rest}`.trim()); out.add(`${p}${letters}${rest}`.trim()) }
    }
  }
  return [...out].filter(Boolean)
}

/** MAM albums filed under a DIFFERENT spelling of this catalogue. */
async function variantAlbums(db, cat) {
  const seen = new Set([S(cat).toUpperCase()])
  const hits = []
  for (const v of catalogueVariants(cat)) {
    if (seen.has(v.toUpperCase())) continue
    seen.add(v.toUpperCase())
    const rows = await db.find(ALBUMS_LAYOUT, [{ 'Album Catalogue Number': '==' + v }, { 'Reference Catalogue Number': '==' + v }], 2)
    for (const r of rows) {
      hits.push({
        catalogue: S(r.fieldData['Album Catalogue Number'] || r.fieldData['Reference Catalogue Number']) || v,
        albumID: S(r.fieldData['AlbumID']),
        title: S(r.fieldData['Album Title']),
        artist: S(r.fieldData['Album Artist']),
        trackCount: Number(r.fieldData['Track Count']) || null,
        recordId: r.recordId,
      })
    }
  }
...
// straight after the create, so a created track carries its mp3 links, track
// number and record links rather than quietly losing them.
const AFTER_CREATE_FIELDS = ['Track Number', 'Featured Artist', 'Version', 'Local Genre', 'Sub Genre',
  'Language Code', 'Lyrical Content Rating', 'Publishing Rights Organisation', 'Rights Territories',
  'Audio_S3_URL', 'CMS_Audio_File', 'AI_BPM', 'AI_Key', 'AI_Mood', 'AI_Energy', 'Download_Price',
  'Catalogue_recid', 'CMS_recid', 'Streamer_recid']
...
/**
 * Which MAM songs already hold each of these ISRCs, and on what album. A track
 * "missing" from this album is often present on another — GALP 1471's twelve
 * were all on GSP 032 — and adding it again would duplicate the recording.
 */
async function songsByIsrc(db, isrcs) {
  const wanted = [...new Set(isrcs.filter(has).map(i => S(i).toUpperCase()))]
  const out = new Map()
  const albumCache = new Map()
  for (let i = 0; i < wanted.length; i += 40) {
    const chunk = wanted.slice(i, i + 40)
    const rows = await db.find(SONGS_LAYOUT, chunk.map(v => ({ ISRC: '==' + v })), 400)
    for (const r of rows) {
      const key = S(r.fieldData['ISRC']).toUpperCase()
      if (!key) continue
      const albumID = S(r.fieldData['AlbumID'])
      if (!albumCache.has(albumID)) {
        const a = albumID ? (await db.find(ALBUMS_LAYOUT, [{ AlbumID: '==' + albumID }], 1))[0] : null
        albumCache.set(albumID, S(a?.fieldData?.['Album Catalogue Number'] || a?.fieldData?.['Reference Catalogue Number']) || S(r.fieldData['Album Catalogue']) || albumID)
      }
      const list = out.get(key) || []
      list.push({ albumID, catalogue: albumCache.get(albumID), track: S(r.fieldData['Track Name']) })
      out.set(key, list)
    }
  }
  return out
...
export async function planMamFill(catalogue, { precedence = DEFAULT_PRECEDENCE, askedFor = null } = {}) {
  ...
  try {
    ...
    // ── album level ───────────────────────────────────────────────────────
    // A missing track that already exists elsewhere in MAM is flagged, not
    // offered blind: adding it would be a second copy of the same recording.
    const missingList = [...missingByTrack.values()].sort((a, b) => (Number(a.seq) || 999) - (Number(b.seq) || 999))
    if (missingList.length) {
      const elsewhere = await songsByIsrc(db, missingList.map(m => m.isrc))
      const mine = new Set([S(album.fieldData?.['AlbumID'])])
      for (const m of missingList) {
        const hits = (elsewhere.get(S(m.isrc).toUpperCase()) || []).filter(h => !mine.has(h.albumID))
        if (hits.length) m.elsewhere = hits.slice(0, 4)
      }
    }
