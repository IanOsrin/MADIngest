...
export async function createGalloRecord(metadata) {
  ...
  if (metadata.asset_number)      fieldData['Filename']               = metadata.asset_number
  if (metadata.wav_filename)      fieldData['Filename.wav']           = metadata.wav_filename
...
function mapGalloRecord(record) {
  ...
  const wavFilename  = pick(f, ['Filename.wav', 'Filename .wav', 'Audio Filename'])
