...
// `options.timeoutMs` overrides FM_TIMEOUT_MS for a single call, and ABORTS the
// request rather than abandoning it — an abandoned FM request still holds its
// connection. Used by the cross-DB search, which puts a user behind the request
// and cannot afford one slow database's 60s budget.
async function galloFetch(path, options = {}) {
  const { timeoutMs = FM_TIMEOUT_MS, ...fetchOpts } = options
  ...
  try {
    ...
    if (res.status === 401) {
      ...
      const { signal: s2, clear: c2 } = withTimeout(timeoutMs)
      try {
        return await fetch(url, {
          ...fetchOpts,
          signal: s2,
          headers: { Accept: 'application/json', Authorization: `Bearer ${fresh}`, ...(fetchOpts.headers || {}) }
        })
      } finally { c2() }
    }
    return res
...
export async function createGalloRecord(metadata) {
  ...
  //
  // TWO genre fields, two different jobs — do not merge them:
  //   Genre       → DDEX/Ingrooves. Their controlled vocabulary, their exact
  //                 spelling ("afro-folk"). Written through verbatim, NEVER
  //                 normalised, or deliveries break.
  //   Local Genre → MadStreamer. Ours, and held to the 45-value taxonomy.
  //
  // Derived here rather than at the call sites because a dozen paths create
  // Gallo records; doing it once means new records can't add to the 12,657
  // blank Local Genres that force the sync to fall back on the Ingrooves value.
  if (metadata.genre)         fieldData['Genre']                     = metadata.genre
  {
    const lg = normalizeGenre(metadata.local_genre ?? metadata.genre)
    if (lg)                   fieldData['Local Genre']               = lg
  }
  ...
  // Canonical Vision master reference — '/bucket/key' (NFC), the same format
  // resolveGalloAudio reads first. The layout filter below drops it if the
  // layout predates the Audio_URL field.
  if (metadata.audio_url) fieldData['Audio_URL'] = String(metadata.audio_url).normalize('NFC')

  // Technical sound-info block (the Media_GetSoundInfo plugin's old output,
  // now read off the Vision WAV header). Field capitalisation varies — send
  // both; the layout filter keeps whichever exists.
  if (metadata.audio_details) { fieldData['Audio details'] = metadata.audio_details
                                fieldData['Audio Details'] = metadata.audio_details }
  ...
  if (process.env.GALLO_CREATE_VERIFY !== 'false') {
    try {
      ...
      if (!verifyRes.ok) {
        ...
        // Audio_URL is only checked when we sent it — FM silently discards
        // writes to fields missing from the layout, which is exactly the
        // failure this read-back exists to catch.
        const wantAudio = payload['Audio_URL']
        const gotAudio  = f['Audio_URL']
        const audioOk   = wantAudio == null || gotAudio === wantAudio
        const ok = gotCat === wantCat && gotIsrc === wantIsrc && gotTitle === wantTitle && audioOk
        if (ok) {
          console.log(`[Gallo create verify] ✓ rec ${fmRecordId} — cat="${gotCat}" isrc="${gotIsrc}" title="${gotTitle}"${wantAudio ? ` audio_url ✓` : ''}`)
          _recordCreateEvent({
          ...
          if (!audioOk) console.warn(`    Audio_URL               sent="${wantAudio}" stored="${gotAudio}"`)
          _recordCreateEvent({
            phase: 'verify_mismatch', recordId: fmRecordId,
            sent:   { catalogue: wantCat, isrc: wantIsrc, title: wantTitle, audio_url: wantAudio },
            stored: { catalogue: gotCat,  isrc: gotIsrc,  title: gotTitle,  audio_url: gotAudio },
          })
        }
      }
    } catch (e) {
...
// ── Artwork table ────────────────────────────────────────────────────────────
// Ian's manual flow is: new record in Artwork, type the catalogue number, drag
// the image into the container. This does the same three steps. The image is
// PUSHED INTO the container rather than referenced by URL — unlike Audio_URL,
// which points at the Vision master — because that is how the table is used.
const ARTWORK_LAYOUT = process.env.GALLO_FM_ARTWORK_LAYOUT || 'Artwork'

/** Existing Artwork records for a catalogue — so an import cannot duplicate them. */
export async function findArtworkByCatalogue(catalogueNo) {
  if (!base || !catalogueNo) return []
  const res = await galloFetch(`/layouts/${encodeURIComponent(ARTWORK_LAYOUT)}/_find`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ query: [{ 'Catalogue Number': `==${catalogueNo}` }], limit: 5 }),
  })
  if (res.status === 404) return []           // FM's "no records match"
  const json = await res.json().catch(() => ({}))
  if (!res.ok) return []
  return (json?.response?.data || []).map(r => ({ recordId: String(r.recordId), fieldData: r.fieldData }))
}

export async function createArtworkRecord({ catalogue_no, image, filename, contentType }) {
  if (!base) throw new Error('GALLO_FM_* env vars not configured')
  if (!catalogue_no) throw new Error('catalogue_no is required for an Artwork record')

  // ONLY the catalogue number. 'Resource reference' is auto-enter on this
  // layout — FileMaker issues the GMVi serial itself (GMVi7057, 7058 …) — and
  // supplying a value overrides it, which is what happened on record 7712: it
  // got the DDEX reference "A1" instead of a GMVi number. Barcode is left
  // empty because existing records leave it empty. Send nothing FileMaker
  // wants to fill in for itself.
  const fieldData = { 'Catalogue Number': catalogue_no }

  const known = await getGalloLayoutFieldSet(ARTWORK_LAYOUT).catch(() => null)
  if (known) {
    for (const k of Object.keys(fieldData)) {
      if (!known.has(k)) { console.warn(`[Gallo artwork] "${k}" not on ${ARTWORK_LAYOUT} — dropped`); delete fieldData[k] }
    }
  }

  const res = await galloFetch(`/layouts/${encodeURIComponent(ARTWORK_LAYOUT)}/records`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fieldData }),
  })
  const json = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(`Artwork record create failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  const recordId = String(json?.response?.recordId || '')

  // The container upload is a separate request. No Content-Type header here —
  // fetch must set it itself so the multipart boundary is correct.
  if (image && image.length) {
    const form = new FormData()
    form.append('upload', new Blob([image], { type: contentType || 'image/jpeg' }), filename || 'cover.jpg')
    const up = await galloFetch(
      `/layouts/${encodeURIComponent(ARTWORK_LAYOUT)}/records/${recordId}/containers/Picture/1`,
      { method: 'POST', body: form }
    )
    const upJson = await up.json().catch(() => ({}))
    if (!up.ok) throw new Error(`Artwork image upload failed: ${upJson?.messages?.[0]?.message || `HTTP ${up.status}`}`)
  }

  return { recordId, layout: ARTWORK_LAYOUT, bytes: image ? image.length : 0 }
}

/**
 * Upload (or replace) the Picture container on an EXISTING Artwork record.
 * createArtworkRecord handles the create+upload path; this covers the Album
 * tab's "replace cover" on records that already exist.
 */
export async function uploadArtworkImage(recordId, image, filename, contentType) {
  if (!base) throw new Error('GALLO_FM_* env vars not configured')
  if (!recordId) throw new Error('uploadArtworkImage: recordId required')
  if (!image?.length) throw new Error('uploadArtworkImage: empty image')
  const form = new FormData()
  form.append('upload', new Blob([image], { type: contentType || 'image/jpeg' }), filename || 'cover.jpg')
  const res = await galloFetch(
    `/layouts/${encodeURIComponent(ARTWORK_LAYOUT)}/records/${recordId}/containers/Picture/1`,
  ...
  const json = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(`Artwork image upload failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  return { recordId: String(recordId), bytes: image.length }
}
...
export async function createTapeFileRecord(metadata) {
  ...
  if (metadata.catalogue_no)   fieldData['Album Catalogue Number']     = metadata.catalogue_no
  // Send every spelling the field might have — introspection drops the ones
  // that don't exist. API_Tape_Files calls these 'Bar Code' (with a space) and
  // 'Year_Release_num', which is why 'Barcode' and 'Year of Release' were
  // silently discarded on every import. Aliasing here rather than renaming the
  // FileMaker fields, which other layouts and scripts may depend on.
  if (metadata.barcode)      { fieldData['Barcode']                    = String(metadata.barcode)
                               fieldData['Bar Code']                   = String(metadata.barcode)
                               fieldData['UPC']                        = String(metadata.barcode) }
  if (metadata.year)         { fieldData['Year of Release']            = String(metadata.year)
                               fieldData['Year_Release_num']           = String(metadata.year) }
  if (metadata.release_date)   fieldData['Release Date']               = metadata.release_date
  if (metadata.original_release_date) fieldData['Original Release date'] = metadata.original_release_date
  // Same two-field split as createGalloRecord above: Genre verbatim for
  // Ingrooves, Local Genre normalised for MadStreamer.
  if (metadata.genre)          fieldData['Genre']                      = metadata.genre
  {
    const lg = normalizeGenre(metadata.local_genre ?? metadata.genre)
    if (lg)                    fieldData['Local Genre']                = lg
 ...
 */
/**
 * Every record whose `Local Genre` is empty, paged.
 *
 * FileMaker's "=" operator with nothing after it means "field is empty", so FM
 * does the filtering — no scanning 43,785 records client-side to find the
 * 12,657 that matter. Returns raw FM records; the caller groups them.
 *
 * NOTE: only `Local Genre` is considered. A record can have a perfectly good
 * `Genre` (the Ingrooves value) and still appear here — that is intended, the
 * two fields are separate vocabularies serving different consumers.
 */
export async function galloFindEmptyLocalGenre({ pageSize = 1000, maxRecords = 40000 } = {}) {
  const out = []
  let offset = 1
  while (out.length < maxRecords) {
    const res = await galloFetch(`/layouts/${encodeURIComponent(GALLO_FM_LAYOUT)}/_find`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: [{ 'Local Genre': '=' }], limit: pageSize, offset })
    })
    const json = await res.json().catch(() => ({}))
    // 401 = "no records match" in FM's error vocabulary, i.e. we are done.
    if (!res.ok) {
      const code = json?.messages?.[0]?.code
      if (String(code) === '401') break
      throw new Error(`Gallo empty-genre find failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
    }
...
function mapGalloRecord(record) {
  ...
  return {
    ...
    // `genre` stays Genre-first: it feeds DDEX/Ingrooves, whose controlled
    // vocabulary requires their exact spelling (e.g. "afro-folk"). Do not
    // normalise it and do not reorder this pick.
    genre:                 pick(f, ['Genre', 'Local Genre']),
    // `local_genre` is the MadStreamer-facing value and OURS to normalise.
    // Exposed separately so the Streamer sync can prefer it — reading `genre`
    // there is what pushed Ingrooves spellings into Streamer's Local Genre.
    local_genre:           pick(f, ['Local Genre']),
    ...
    // Canonical Vision reference (the 24-bit master), populated by the audio
    // linker. Preferred over the mount-era container for playback + DDEX.
    audio_url_ref:         pick(f, ['Audio_URL', 'audio_Url', 'Audio_Url', 'audio_url']),
 ...
 */
/**
 * Which of these catalogues already have Gallo records — ONE find for the lot.
 *
 * The CCA scan previously issued a query per release; a 141-release batch meant
 * 141 round trips just to answer "is this already imported?". FileMaker's _find
 * treats multiple query objects as OR, so the whole page costs one request.
 */
export async function findGalloCataloguesPresent(catalogueNos) {
  if (!base) throw new Error('GALLO_FM_* env vars not configured')
  const uniq = [...new Set((catalogueNos || []).map(c => String(c || '').trim()).filter(Boolean))]
  ...
  const res = await galloFetch(
    `/layouts/${encodeURIComponent(GALLO_FM_LAYOUT)}/_find`,
    {
      ...
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
        query: uniq.map(c => ({ 'Album Catalogue Number': `==${c}` })),
        limit: 5000,
      }),
    }
  )
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
    if (json?.messages?.[0]?.code === '401') return new Map()   // no matches
    throw new Error(`Gallo bulk catalogue find failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }

  // Count per catalogue so the caller can report "already has N records".
  const counts = new Map()
  for (const r of (json?.response?.data || [])) {
    const c = String(r.fieldData?.['Album Catalogue Number'] || '').trim()
...
/**
 * The same find, but the RAW fieldData. mapGalloRecord keeps only what the DDEX
 * pipeline needs; a field-by-field comparison against another database needs
 * everything the layout holds.
 */
export async function findGalloRawByCatalogue(catalogueNo) {
  if (!base) throw new Error('GALLO_FM_* env vars not configured')
  const res = await galloFetch(
    `/layouts/${encodeURIComponent(GALLO_FM_LAYOUT)}/_find`,
    { method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  if (!res.ok) {
    ...
    throw new Error(`Gallo FM find failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }
  return (json?.response?.data || []).map(r => ({ recordId: String(r.recordId || ''), fieldData: r.fieldData || {} }))
}
 ...
 */
export async function searchGalloRecords(term, limit = 50, offset = 0, { timeoutMs } = {}) {
  if (!base) throw new Error('GALLO_FM_* env vars not configured')
  if (!GALLO_FM_LAYOUT) throw new Error('GALLO_FM_LAYOUT not set')
    ...
    `/layouts/${encodeURIComponent(GALLO_FM_LAYOUT)}/_find`,
    {
      ...
      body:    JSON.stringify({
        query: [
          ...
          { 'Album Title':           `*${t}*` },
          { 'ISRC':                  `*${t}*` },
          // Catalogue spacing differs across DBs — search every variant.
          ...catSearchVariants(t).flatMap(v => [
            { 'Album Catalogue Number': `*${v}*` },
            { 'Reference Catalogue Number': `*${v}*` },
          ]),
        ],
        limit,
  ...
  const tracks = (json?.response?.data || []).map(record => {
    ...
    return {
      ...
      local_genre:   f['Local Genre'] || null,   // Streamer-facing; `genre` above stays Ingrooves-facing
 ...
 */
/** Raw fieldData for one Gallo record by FM recordId, or null if it's gone.
 *  Used by the audio endpoint to resolve a record's Vision location. */
export async function getGalloFieldData(recordId) {
  if (!base) throw new Error('GALLO_FM_* env vars not configured')
  const res = await galloFetch(`/layouts/${encodeURIComponent(GALLO_FM_LAYOUT)}/records/${encodeURIComponent(recordId)}`)
  const json = await res.json().catch(() => ({}))
  if (!res.ok) {
    if (String(json?.messages?.[0]?.code) === '101') return null // record missing
    throw new Error(`Gallo getGalloFieldData failed: ${json?.messages?.[0]?.message || `HTTP ${res.status}`}`)
  }
  return json?.response?.data?.[0]?.fieldData || null
}
