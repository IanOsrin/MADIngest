...
export function buildDdexPackage({ tracks, audioBufs, artworkBuf, artworkExt = 'jpg', overrides = {} }) {
  if (!Array.isArray(tracks) || tracks.length === 0) {
    throw new Error('No tracks supplied')
  }
  ...
  if (!Buffer.isBuffer(artworkBuf)) {
    throw new Error('artworkBuf is required (front cover)')
  }

  // Sort tracks by sequence (treat missing as the order they came in)
  const sorted = tracks
  ...
  // Album-level data — derive from the first track unless overridden.
  // Only safe to reach this point because validatePackage above has already
  // confirmed Year is a 4-digit number and any supplied dates parse cleanly.
  const head = sorted[0]
  const validYear = /^\d{4}$/.test(String(head.year || '').trim()) ? String(head.year).trim() : null
  if (!validYear) {
    throw new Error(`Cannot build XML: track Year "${head.year}" is not a 4-digit year`)
  }
  ...
  // Per-track derived references + filenames
  sorted.forEach((t, i) => {
    ...
    t._duration    = t.duration_sec
                       ? secsToISODuration(t.duration_sec)
                       : null
