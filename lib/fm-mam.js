import { creditNames } from './credits.js'
...
async function session() {
  ...
  return {
    ...
    getById: async (layout, recordId) => {
      const res = await fetch(`${base}/layouts/${encodeURIComponent(layout)}/records/${encodeURIComponent(recordId)}`, { headers: H })
      const j2 = await res.json()
      if (j2?.messages?.[0]?.code === '101') return null       // no such record
      if (!j2?.response?.data) throw new Error(`${layout} read failed: ${j2?.messages?.[0]?.message}`)
      return j2.response.data[0]?.fieldData || null
    },
...
function mapMamRecord(s, a) {
  ...
  // MAM holds credits in two shapes: numbered singular slots (Composer,
  // Composer 2 …) and a plural field carrying a semicolon-separated list
  // (Composers). Every WRITE path — createMamSong, cache-db-sync's builders —
  // fills both, but albums merged in from the three source databases often
  // have only the plural one, and reading just the singular slots rendered
  // their credits as blank on the MAM tab and shipped DDEX with no
  // IndirectResourceContributor at all (TGE 90, 2026-09-17).
  //
  // Both are read and the result deduped: where both shapes are populated they
  // hold the SAME value, so a naive concat printed every name twice.
  //
  // The credit can also arrive in Ingrooves' own form, role tags and all — TGE
  // 90's Composers field is literally "Pieter W. Grobbelaar <Lyricist>, Pieter W.
  // Grobbelaar <Composer>", and its Composer / Composer 2 slots hold one tagged
  // name each. Left as-is that rendered the tags on the MAM tab and shipped a
  // single writer called "…<Lyricist>, …<Composer>" in DDEX. So role tags are
  // stripped, and a tagged value is also split on commas. An untagged value is
  // split on ';' only: "Mankwane, Marks" is one name written surname-first.
  const creditList = creditNames

  const composers = creditList(s['Composer'], s['Composer 2'], s['Composer 3'], s['Composer 4'], s['Composers'])
  // The credit as MAM holds it, role tags included — what the MAM tab shows.
  // `composers` above stays plain names: DDEX and the name slots need those.
  const composersCredit = val(s['Composers']) || composers.join('; ') || null
  const producers = creditList(s['Producer'], s['Producers'])
  ...
  return {
    ...
    composers,
    composers_credit:      composersCredit,
    producers,
 ...
 */
/**
 * Which fields each scope searches. 'artist' and 'title' cover BOTH lanes on
 * purpose: an album is worth finding by its own name or by a track on it, and
 * by the album artist or the artist on a track.
 */
const MAM_SEARCH_SCOPES = {
  any:    { song: ['Track Name', 'Track Artist', 'Album Catalogue', 'ISRC', 'Filename', 'MasterID'],
            album: ['Album Title', 'Album Artist', 'Album Catalogue Number', 'Reference Catalogue Number'] },
  artist: { song: ['Track Artist'], album: ['Album Artist'] },
  title:  { song: ['Track Name'],   album: ['Album Title'] },
}

export async function searchMamRecords(term, { limit = 50, scope = 'any' } = {}) {
  const t = String(term || '').trim()
  if (t.length < 2) return { tracks: [], foundCount: 0 }
  const want = MAM_SEARCH_SCOPES[scope] || MAM_SEARCH_SCOPES.any
  const fm = await session()
  try {
    const [songSample, albumSample] = await Promise.all([fm.getOne(SONGS_LAYOUT()), fm.getOne(ALBUMS_LAYOUT())])
    const songFields  = want.song.filter(f => f in songSample)
    const albumFields = want.album.filter(f => f in albumSample)
...
/**
 * Album search for the MAM tab: albums, not a capped list of tracks.
 *
 * The tab used to group searchMamRecords' tracks by catalogue, but that stops at
 * `limit` tracks and looks at only 5 Albums records, so a prolific artist hid
 * whole albums: "John Edmond" matches 175 tracks, and DKL 48 ("John Edmond and
 * Joanna Field") fell past the 150-track cut (2026-09-17). Here both lanes only
 * collect AlbumIDs — Albums records matched directly, and the albums of matching
 * songs — then the Albums records are fetched in batches. No per-album song
 * finds, so a wide search stays a handful of requests.
 */
export async function searchMamAlbums(term, { scope = 'any', maxSongs = 3000, maxAlbums = 500 } = {}) {
  const t = String(term || '').trim()
  if (t.length < 2) return { albums: [], truncated: false }
  const want = MAM_SEARCH_SCOPES[scope] || MAM_SEARCH_SCOPES.any
  const fm = await session()
  try {
    const [songSample, albumSample] = await Promise.all([fm.getOne(SONGS_LAYOUT()), fm.getOne(ALBUMS_LAYOUT())])
    ...
    const [songHits, albumHits] = await Promise.all([
      songFields.length  ? fm.findOr(SONGS_LAYOUT(),  branches(songFields),  maxSongs)  : { rows: [], foundCount: 0 },
      albumFields.length ? fm.findOr(ALBUMS_LAYOUT(), branches(albumFields), maxAlbums) : { rows: [], foundCount: 0 },
    ])

    const albumById = new Map()
    const hits = new Map()                       // AlbumID → matching songs
    for (const a of albumHits.rows) {
      const id = val(a['AlbumID'])
      if (id) { albumById.set(id, a); hits.set(id, hits.get(id) || 0) }
    }
    for (const s of songHits.rows) {
      const id = val(s['AlbumID'])
      if (id) hits.set(id, (hits.get(id) || 0) + 1)
    }

    // Albums records for song-lane hits, 50 AlbumIDs per OR-find.
    const missing = [...hits.keys()].filter(id => !albumById.has(id))
    for (let i = 0; i < missing.length; i += 50) {
      const { rows } = await fm.findOr(ALBUMS_LAYOUT(), missing.slice(i, i + 50).map(id => ({ AlbumID: '==' + id })), 50)
      for (const a of rows) albumById.set(String(a['AlbumID']), a)
    }

    const albums = [...albumById.entries()]
      .map(([id, a]) => ({
        album_id:     id,
        catalogue:    val(a['Album Catalogue Number']) || val(a['Reference Catalogue Number']) || '',
        album_title:  val(a['Album Title']),
        album_artist: val(a['Album Artist']),
        hits:         hits.get(id) || 0,
      }))
      .filter(a => a.catalogue)
    return {
      albums,
      truncated: songHits.foundCount > songHits.rows.length || albumHits.foundCount > albumHits.rows.length,
    }
  } finally { await fm.close() }
}
...
// ── Album tab support ───────────────────────────────────────────────────────
// The album tab edits MAM in place, which the DDEX read paths above never do.
// Two things it needs that they do not: the recordIds to patch, and the set of
// field names each layout actually has (FM silently discards an unknown field,
// so a typo would look like a save that did nothing).

const _mamFieldSets = new Map()

/**
 * Field names present on a MAM layout, cached for the process.
 *
 * Read from one record rather than /metadata because that is the shape the
 * write paths care about — a field on the layout but absent from every record
 * still comes back in fieldData with an empty value.
 */
export async function getMamLayoutFieldSet(which = 'Songs') {
  const layout = which === 'Albums' ? ALBUMS_LAYOUT() : SONGS_LAYOUT()
  if (_mamFieldSets.has(layout)) return _mamFieldSets.get(layout)
  const fm = await session()
  try {
    const set = new Set(Object.keys(await fm.getOne(layout)))
    if (set.size) _mamFieldSets.set(layout, set)
    return set
  } finally { await fm.close() }
}

/** Drop the cached field sets (after a layout change in FileMaker). */
export function reloadMamLayoutFields() { _mamFieldSets.clear() }

/** Keep only the fields the layout really has. */
export function filterToMamLayout(fieldData, known) {
  if (!known || !known.size) return fieldData
  return Object.fromEntries(Object.entries(fieldData).filter(([k]) => known.has(k)))
}

/** The catalogue spellings MAM might hold: "BL 789", "BL789", "BL 789". */
function catVariants(cat) {
  const c = String(cat || '').trim()
  return [...new Set([c, c.replace(/\s+/g, ''), c.replace(/^([A-Za-z]+)\s*(\d)/, '$1 $2')])].filter(Boolean)
}

/** One MAM album record ({recordId, fieldData}) by catalogue, or null. */
export async function findMamAlbumByCatalogue(catalogueNo) {
  const fm = await session()
  try { return await _findAlbum(fm, catalogueNo) }
  finally { await fm.close() }
}

async function _findAlbum(fm, catalogueNo) {
  for (const c of catVariants(catalogueNo)) {
    for (const field of ['Album Catalogue Number', 'Reference Catalogue Number']) {
      const hits = await fm.findRaw(ALBUMS_LAYOUT(), [{ [field]: '==' + c }], 2)
...
/**
 * One album ready for the album tab: the Albums record, and every song both in
 * the flat display shape AND with its recordId, in track order.
 *
 * mapMamRecord leaves fm_record_id null (the DDEX pipeline has no use for it),
 * so it is filled in here — the tab patches by recordId.
 */
export async function getMamAlbumForEdit(catalogueNo) {
  const fm = await session()
  try {
    const album = await _findAlbum(fm, catalogueNo)
    if (!album) return null
    const songs = await fm.findRaw(SONGS_LAYOUT(), [{ AlbumID: '==' + album.fieldData['AlbumID'] }], 500)
    const tracks = songs
      .map(s => ({ ...mapMamRecord(s.fieldData, album.fieldData), fm_record_id: s.recordId }))
      .sort((a, b) => (a.sequence_no ?? 999) - (b.sequence_no ?? 999))
    return { album, tracks }
  } finally { await fm.close() }
}

/** Albums whose UPC/barcode matches, in the flat track shape. */
export async function findMamRecordsByBarcode(barcode) {
  const b = String(barcode || '').trim()
  if (!b) throw new Error('barcode is required')
  const fm = await session()
  try {
    const albums = await fm.find(ALBUMS_LAYOUT(), { 'UPC': '==' + b }, 5)
    if (!albums.length) return []
    const out = []
    for (const a of albums) {
      const songs = await fm.find(SONGS_LAYOUT(), { AlbumID: '==' + a['AlbumID'] }, 500)
      for (const s of songs) out.push(mapMamRecord(s, a))
    }
    return out
  } finally { await fm.close() }
}

/** Patch one Songs record, keeping only fields the layout has. */
export async function updateMamSong(recordId, fieldData) {
  const known = await getMamLayoutFieldSet('Songs').catch(() => null)
  const payload = filterToMamLayout(fieldData, known)
  if (!Object.keys(payload).length) throw new Error('No writable MAM Songs fields in this edit')
  await updateMamRecord('Songs', recordId, payload)
  return Object.keys(payload)
}

/** Patch one Albums record, keeping only fields the layout has. */
export async function updateMamAlbum(recordId, fieldData) {
  // A cover may only be written if it came through lib/album-cover.js.
  if ('Artwork_S3_URL' in (fieldData || {})) assertPipelineCoverUrl(fieldData.Artwork_S3_URL, 'MAM Albums Artwork_S3_URL')
  const known = await getMamLayoutFieldSet('Albums').catch(() => null)
  const payload = filterToMamLayout(fieldData, known)
  if (!Object.keys(payload).length) throw new Error('No writable MAM Albums fields in this edit')
  await updateMamRecord('Albums', recordId, payload)
  return Object.keys(payload)
}

/** One MAM record's fieldData by recordId ('Songs' or 'Albums'), or null. */
export async function getMamFieldData(which, recordId) {
  const layout = which === 'Albums' ? ALBUMS_LAYOUT() : SONGS_LAYOUT()
  const fm = await session()
  try { return await fm.getById(layout, recordId) }
  finally { await fm.close() }
}

/**
 * One MAM album as FileMaker holds it — raw fieldData, not the flat DDEX shape —
 * for code that has to map field-by-field onto another database (the MAM →
 * MADStreamer sync). Songs in sequence order.
 */
export async function getMamAlbumRaw(catalogueNo) {
  const fm = await session()
  try {
    const album = await _findAlbum(fm, catalogueNo)
    if (!album) return null
    const songs = await fm.findRaw(SONGS_LAYOUT(), [{ AlbumID: '==' + album.fieldData['AlbumID'] }], 500)
    songs.sort((a, b) =>
      (parseInt(a.fieldData['Sequence Number'], 10) || 999) - (parseInt(b.fieldData['Sequence Number'], 10) || 999))
    return { album, songs }
  } finally { await fm.close() }
}
