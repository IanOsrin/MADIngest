...
export function parseDDEXErn41(root, fileMap) {
  ...
  // Canonical track order. DDEX states it in ResourceGroupContentItem, pairing
  // SequenceNumber with a ReleaseResourceReference (A1, A2 …). Those repeat per
  // territory — 34 items for 11 tracks in a real CCA delivery — so first one
  // wins. Nesting varies by profile (ResourceGroup inside ResourceGroup), so
  // this walks the tree rather than assuming a path.
  const seqByRef = new Map()
  ;(function collectSeq(node) {
    if (!node || typeof node !== 'object') return
    if (Array.isArray(node)) { node.forEach(collectSeq); return }
    for (const item of toArray(node.ResourceGroupContentItem)) {
      const ref = str(item?.ReleaseResourceReference)
      const seq = parseInt(str(item?.SequenceNumber), 10)
      if (ref && Number.isFinite(seq) && !seqByRef.has(ref)) seqByRef.set(ref, seq)
    }
    for (const v of Object.values(node)) collectSeq(v)
  })(root)
  ...
  releases.forEach(release => {
      ...
      if (trackReleases.length > 0) {
        trackReleases.forEach((tr, idx) => {
          ...
          if (sr) {
            ...
            if (!track.track_number) track.track_number = seqByRef.get(ref) ?? seqFromFileName(track.file_name) ?? (idx + 1)
            tracks.push(track)
          }
        })
        return
    ...
    // This path previously pushed tracks with no track_number at all — the
    // reason CCA deliveries came through unsequenced, since they carry no
    // TrackReleaseList.
    refs.forEach((ref, idx) => {
      const key = str(ref)
      const sr = resourceMap[key]
      if (!sr) return
      const track = mapSoundRecording41(sr, albumRelease || release, fileMap, artworkBuffer)
      if (!track.track_number) track.track_number = seqByRef.get(key) ?? seqFromFileName(track.file_name) ?? (idx + 1)
      tracks.push(track)
    })
  })
...
function mapSoundRecording41(sr, release, fileMap, artworkBuffer) {
  ...
  // Resource identity, needed by the CCA Import flow. These already exist in
  // the XML and audioFileName was being computed for the buffer lookup and then
  // discarded — they map straight onto the FM fields added 2026-08-03:
  //   FileName                          -> Filename
  //   ResourceReference                 -> Resource Reference
  //   HashSum                           -> AudioHashSum
  //   TechnicalResourceDetailsReference -> Technical Resource
  const hashSum = str(
    techDetails[0]?.File?.HashSum?.HashSum ||
    techDetails[0]?.File?.HashSum ||
    techDetails[0]?.HashSum?.HashSum ||
    techDetails[0]?.HashSum ||
    null
  )
  const technicalResource = str(
    techDetails[0]?.TechnicalResourceDetailsReference ||
    srD.TechnicalResourceDetailsReference ||
    null
  )
  ...
  // Original release date. CCA deliveries do not currently carry one — 0 of 12
  // sampled had OriginalReleaseDate — so this is almost always null and the
  // caller falls back to releaseDate. Read anyway: when a reissue does state
  // its original date, it is the more accurate value and should win.
  const originalReleaseDate = str(
    srD.OriginalReleaseDate || sr.OriginalReleaseDate ||
    relD.OriginalReleaseDate || release?.OriginalReleaseDate ||
    relD.GlobalOriginalReleaseDate || release?.GlobalOriginalReleaseDate ||
    null
  )
  ...
  return {
    ...
    resource_reference: str(sr.ResourceReference),
    file_name:     audioFileName,
    hash_sum:      hashSum,
    technical_resource: technicalResource,
    pline_text:    str(pLine?.PLineText || null),
    cline_text:    str(cLine?.CLineText || null),
...
// Last-resort sequence: CCA names files <barcode>_<seq>_<seq>.wav, so the first
// numeric group after the barcode is the track number.
function seqFromFileName(name) {
  const m = String(name || '').match(/_(\d{1,3})_\d{1,3}\.[a-z0-9]+$/i)
