...
export async function updateRow(index, patch, expect = {}, { albumWide = false } = {}) {
  return _enqueueWrite(async () => {
    ...
    // Album-wide: apply the album-level keys of the patch to every row that
    // shares this row's catalogue — matched on the PRE-edit value, since the
    // patch may be changing the catalogue itself. Track-level keys in the same
    // patch still land only on the edited row.
    let albumRows = 0
