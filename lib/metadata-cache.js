...
function _parse(buffer) {
  ...
  // The Ingrooves export has TWO "#" columns: a leading running row counter, and
  // the real per-album track number sitting just after "Disc #". SheetJS renames
  // the duplicate header to "#_1". Older exports had only the track-number one,
  // which is why reading '#' used to be right — and why it silently became a row
  // index when the counter column appeared. Decide once per file rather than per
  // row, so a blank track number can never fall through to the counter.
  //
  // Verified 2026-08-03 against 42,978 unambiguous ISRCs: the live cache matches
  // '#_1' on 100% of rows and '#' on 0.0%.
  const seqKey = raw.some(r => Object.prototype.hasOwnProperty.call(r, '#_1')) ? '#_1' : '#'
  const seqOf = (r) => {
    for (const k of [seqKey, 'Track #', 'Seq']) {
      const v = r[k]
      if (v != null && v !== '') return parseInt(v, 10) || null
    }
    return null
  }

  // Duration arrives either as text ("00:04:50") or, from .xlsb exports, as an
  // Excel serial — a fraction of a day, so 0.0033564814814814816 is 4m50s. A
  // real duration is always < 1 day; anything else is passed through untouched
  ...
  return raw.map(r => ({
    ...
    seq:            seqOf(r),
    duration:       durationOf(r['Duration'] ?? r['duration']),
...
// One load at a time: the boot-time fire-and-forget call and the first
// /metadata/rows request used to each download the full S3 JSON in parallel.
// Concurrent callers now share the in-flight load instead.
let _loadPromise = null
export function loadMetadata() {
  if (_loadPromise) return _loadPromise
  _loadPromise = _loadMetadataOnce().finally(() => { _loadPromise = null })
  return _loadPromise
}

async function _loadMetadataOnce() {
  // Never re-read the store while an edit is still uploading — the S3 object
  // would be pre-edit and reloading it would silently discard the edit.
  while (_persistRunning || _persistDirty) {
...
export function searchMetadata(term, limit = 20) {
  ...
  // Catalogue spacing varies ("HUL 40238" vs "HUL40238") — compare space-free.
  const tCat = t.replace(/\s+/g, '')
  const out = []
  ...
  for (const row of _rows) {
    ...
    if (
      ...
      isrc.includes(tUp)     ||
      cat.includes(t)        ||
      (tCat && cat.replace(/\s+/g, '').includes(tCat)) ||
      album.includes(t)      ||
      art.includes(t)        ||
      trk.includes(t)        ||
      tArt.includes(t)
...
export function getStatus() {
  return {
    loaded: _loaded, count: _rows.length, loadedAt: _loadedAt, source: _loadedFrom, s3Key: META_S3_KEY,
    persistPending: _persistRunning || _persistDirty,
...
const _FIELD_TO_COL = {
  ...
  // Write side emits a SINGLE '#'; on re-import seqKey detection sees no '#_1'
  // and correctly falls back to it. Don't rename this to match the export.
  seq:            '#',
  duration:       'Duration',
  genre:          'Track genre',
...
// Fields that describe the ALBUM rather than one track. Editing one of these
// on any row is meant to change the whole album — updateRow's albumWide option
// and the Cache Viewer's push panel both key off this list.
export const ALBUM_FIELD_KEYS = [
  'album_title', 'album_artist', 'release_date', 'original_release_date',
...
// Background persist for INTERACTIVE edits (single-cell update/delete).
// Measured: one S3 rewrite of the 70k-row store is ~43MB and takes ~70s —
// far too long to hold a cell save open. The edit applies in memory at once;
// the upload runs behind it, and edits made while one upload is in flight
// coalesce into a single follow-up upload. Bulk paths (merge/replace/append)
// still persist synchronously — they're rare and worth the wait.
let _persistRunning = false
let _persistDirty   = false
let _persistError   = null    // last background persist failure, if any
...
async function _drainPersist() {
  if (_persistRunning || !_persistDirty) return
  _persistRunning = true
  _persistDirty   = false
  try {
    const where = await _persistAll()
    _persistError = null
    console.log(`[Metadata] Background persist complete (${where})`)
  } catch (e) {
    _persistError = e.message
    console.warn('[Metadata] Background persist FAILED — edits are in memory only:', e.message)
  } finally {
    _persistRunning = false
    if (_persistDirty) _drainPersist()
  }
}
 ...
 */
export async function updateRow(index, patch, expect = {}, { albumWide = false } = {}) {
  return _enqueueWrite(async () => {
    // A cold server (fresh boot, cache still coming down from S3) has an
    // empty _rows — without this wait every edit got "No cache row at index
    // N" and the viewer bounced the value back. Same guard _writeRowsToFile
    // has always had. The expect-check below still protects against real
    // index drift.
    if (!_loaded) await loadMetadata()
    ...
    const albumRows = _applyPatchWithFanout(row, patch, albumWide)
    _schedulePersist()
    _rebuildIndexes()
    _loadedAt = new Date()
    return { ok: true, row, albumRows, persisted: 'background' }
  })
}

// Apply `patch` to `row`; when albumWide, fan the album-level keys out to
// every row sharing the (PRE-edit) catalogue. Returns the sibling count.
function _applyPatch(target, patch, keys) {
  for (const [k, v] of Object.entries(patch || {})) {
    if (!CACHE_FIELD_KEYS.includes(k)) continue
    if (keys && !keys.includes(k)) continue
    if (k === 'seq') target[k] = (v === '' || v == null) ? null : (parseInt(v, 10) || null)
    else             target[k] = (v === '' || v == null) ? null : String(v)
  }
}
function _applyPatchWithFanout(row, patch, albumWide) {
  let albumRows = 0
  if (albumWide && row.catalogue) {
    const albumKeys = Object.keys(patch || {}).filter(k => ALBUM_FIELD_KEYS.includes(k))
    if (albumKeys.length) {
      const key = _normCat(row.catalogue)
      for (const r of _rows) {
        if (r !== row && r.catalogue && _normCat(r.catalogue) === key) {
          _applyPatch(r, patch, albumKeys)
          albumRows++
        }
      }
    }
  }
  _applyPatch(row, patch)
  return albumRows
}

/**
 * Apply MANY row patches in one write (Excel-style fill-down / paste in the
 * Cache Viewer). Same semantics as updateRow per entry — expect-guard,
 * albumWide fanout — but a single persist covers the lot. Per-row failures
 * don't abort the batch; each entry reports its own outcome.
 */
export async function updateRowsBulk(edits = [], { albumWide = false } = {}) {
  return _enqueueWrite(async () => {
    if (!_loaded) await loadMetadata()
    const results = []
    let applied = 0
    for (const { index, patch, expect } of edits) {
      const row = _rows[index]
      if (!row) { results.push({ index, ok: false, error: `No cache row at index ${index}` }); continue }
      if (!_expectMatches(row, expect || {})) {
        results.push({ index, ok: false, error: 'Row changed since the viewer loaded' }); continue
      }
      if (!patch || !Object.keys(patch).length) { results.push({ index, ok: false, error: 'empty patch' }); continue }
      const albumRows = _applyPatchWithFanout(row, patch, albumWide)
      applied++
      results.push({ index, ok: true, albumRows })
    }
    if (applied) {
      _schedulePersist()
      _rebuildIndexes()
    ...
    return { ok: true, applied, results, persisted: 'background' }
  })
}
...
/**
 * Delete a found set — the rows a Cache Viewer search/filter has narrowed to.
 *
 * Indices are array positions, and every splice shifts everything after it, so
 * deleting a set one call at a time silently removes the wrong rows. This does
 * the whole set inside ONE write-queue slot: verify first, then splice in
 * DESCENDING order so no surviving index moves before it is used, then persist
 * and rebuild once rather than per row.
 *
 * `expect` is keyed by index — the same optimistic check deleteRow makes, so a
 * viewer left open while someone else edited the cache is refused rather than
 * deleting whatever has since moved into those positions.
 *
 * Returns the deleted rows. The caller is expected to offer them as a
 * spreadsheet: the cache persists straight to S3, so this is the only undo
 * there is, and 70k rows is far too much to re-key by hand.
 */
export async function deleteRows(indices = [], expect = {}) {
  return _enqueueWrite(async () => {
    if (!_loaded) await loadMetadata()

    const idx = [...new Set(indices.map(n => parseInt(n, 10)).filter(Number.isInteger))]
    if (!idx.length) throw new Error('No rows selected')
    const bad = idx.filter(i => i < 0 || i >= _rows.length)
    if (bad.length) throw new Error(`Index out of range: ${bad.slice(0, 5).join(', ')}`)

    // Verify BEFORE removing anything — a partial delete of a set the user
    // reviewed as a whole is worse than refusing the lot.
    const mismatched = idx.filter(i => !_expectMatches(_rows[i], expect[String(i)] || {}))
    if (mismatched.length) {
      throw new Error(
        `${mismatched.length} row(s) changed since the viewer loaded — reload the Cache Viewer and search again`)
    }

    const removed = []
    for (const i of idx.sort((a, b) => b - a)) removed.push(..._rows.splice(i, 1))

    _schedulePersist()
    _rebuildIndexes()
    _loadedAt = new Date()
    return { ok: true, deleted: removed.length, count: _rows.length, rows: removed.reverse() }
  })
}
...
export async function deleteRow(index, expect = {}) {
  return _enqueueWrite(async () => {
    ...
    _rebuildIndexes()
    _loadedAt = new Date()
    return { ok: true, count: _rows.length, persisted: 'background' }
  })
}
  ...
  const rows = raw.map(r => {
    ...
    for (const { source, target } of mapping) {
      ...
      if (target === 'seq') {
        ...
        // Two source columns hitting the same target get joined, which is how
        // multi-value fields are meant to work (Composer 1..4, Producer 1..3).
        // But when both columns hold the SAME value the join produced nonsense:
        // a sheet with "Album Catalogue Number" and "Reference Catalogue Number"
        // both reading "CDGMP 40296" cached a catalogue of
        // "CDGMP 40296; CDGMP 40296", which no catalogue lookup can ever match,
        // so Add Album could not see the album at all (Ian, 2026-08-06).
        // Keep one copy of a value that is already there.
        const seen = out[target].split(';').map(s => s.trim().toLowerCase())
        if (!seen.includes(val.trim().toLowerCase())) out[target] += '; ' + val
      }
    }
...
/**
 * Cache catalogues that look like variants of `cat` but are not it.
 *
 * The same release is filed differently by format and by pressing: MAM holds
 * "CDGMP 1004" while the cache has the same five Stimela tracks under
 * "GMP 1004D". An exact lookup finds nothing and the album looks as though the
 * cache knows nothing about it.
 *
 * SUGGESTIONS ONLY — this never decides. CDGMP 1823 ("Nelson Mandela's
 * Lifetime Tribute") and GMP 1823 ("Ziyangiluma Izinja") normalise to the same
 * key and are different records, so the caller must show these to a human and
 * let them choose. Each candidate carries its album title precisely so that
 * choice can be made on sight.
 *
 * Leading zeros are significant and kept: CDGSP 006 and CDGSP 6 are different
 * releases.
 */
const _CAT_FORMATS = ['CASS', 'CDS', 'CD', 'MC', 'LP', 'K7']

function _catFamilyKey(s) {
  let k = String(s || '').toUpperCase().replace(/[^A-Z0-9]/g, '')
  for (const f of _CAT_FORMATS) {
    if (k.startsWith(f) && k.length - f.length >= 2 && /[A-Z]/.test(k[f.length])) { k = k.slice(f.length); break }
  }
  // Trailing letters after the number mark a pressing or edition (1004D, 610R,
  // 41058DD) rather than a different release.
  return k.replace(/(\d)[A-Z]{1,2}$/, '$1')
}

export function findCatalogueVariants(cat) {
  const target = _catFamilyKey(cat)
  const exact  = String(cat || '').trim().toLowerCase()
  if (!target) return []
  const out = []
  for (const [key, rows] of _byCatAll) {
    if (key === exact) continue                 // the exact match is not a variant
    if (_catFamilyKey(key) !== target) continue
    const first = rows[0] || {}
    out.push({
      catalogue:  String(first.catalogue || key).trim(),
      albumTitle: String(first.album_title || '').trim(),
      artist:     String(first.album_artist || '').trim(),
      trackCount: rows.length,
    })
  }
  return out.sort((a, b) => b.trackCount - a.trackCount)
}
