...
async function _writeRowsToFile(rows) {
  // JSON store (Docker/Render): rows are already in internal field-name
  // format, so append and rewrite the JSON file directly — no SheetJS.
  if (META_IS_JSON) {
    let existing = []
    try { existing = JSON.parse(await readFile(META_FILE, 'utf8')) }
    catch { /* file doesn't exist yet — start fresh */ }
    await writeFile(META_FILE, JSON.stringify([...existing, ...rows]))
    return loadMetadata()
  }

  // XLSX store (local dev): read all existing rows via _parse()/_toXlsxRow()
  // so they are normalised to the canonical 19-column XLSX_HEADERS format
  // regardless of how the source file was structured (e.g. 'Album title' vs
  // 'Album Title'). Writing raw sheet_to_json output was the root cause of
  // the previous 34-column drift.
  let existingRows = []
  try {
