...
export function checkDataHealth(entries, { primaryKey = 'madstreamer' } = {}) {
  ...
  for (const src of entries) {
    ...
    const shared = [...byIsrc.entries()]
      .filter(([, list]) => new Set(list.map(t => norm(t.title))).size > 1)
    if (shared.length) {
      const worst = shared.sort((a, b) => b[1].length - a[1].length)[0]
      add('error', 'isrc-shared',
        `${src.label}: ${plural(shared.length, 'ISRC')} on more than one song`,
        `${worst[0]} is on ${worst[1].length} different titles — ${worst[1].slice(0, 3).map(t => `"${t.title}"`).join(', ')}${worst[1].length > 3 ? '…' : ''}. They collapse onto one row here, and royalty reporting keys on this field.`,
        src.key)
    }
