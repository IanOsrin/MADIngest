...
import { S3Client, ListBucketsCommand, ListObjectsV2Command, GetObjectCommand, DeleteObjectsCommand, CopyObjectCommand } from '@aws-sdk/client-s3'
...
function s3Parse(rel) {
  ...
  // Traversal is a ".." SEGMENT, not the characters appearing anywhere. Testing
  // the whole string rejected files merely NAMED with two dots — Gallo has
  // several ("SOUL BROTHERS-THUL' UBHEKE..doc", "KW.. MEDIA RELEASE.docx") and
  // they were unreachable through every Vision path: browse, download, import
  // (Ian, 2026-08-07). normalize() above has already resolved any real ".."
  // against the root, so a surviving segment would be an escape attempt.
...
/**
 * Upload a local file to a Vision path. The ONLY write operation on the
 * Vision drive — it exists for the add-only upload tab, and callers MUST
 * check the key doesn't already exist (visionStat) before calling: S3 puts
 * replace silently, and Vision holds the master audio.
 */
export async function visionUploadFile(rel, localPath, contentType, onProgress) {
  if (PROTO !== 's3') throw new Error('visionUploadFile requires the s3 protocol')
  const { bucket, key } = s3Parse(rel)
  if (!bucket || !key) throw new Error('Not a file path')
  const up = new Upload({
    client: s3(),
    ...
    params: { Bucket: bucket, Key: key, Body: createReadStream(localPath), ContentType: contentType || 'application/octet-stream' },
  })
  if (onProgress) up.on('httpUploadProgress', (e) => onProgress(e.loaded ?? 0))
  await up.done()
}

/** Every object key under a prefix (or the single key itself if it is a file). */
async function keysUnder(rel) {
  const { bucket, key } = s3Parse(rel)
  if (!bucket || !key) throw new Error('Refusing to operate on a whole bucket')
  const exact = await visionStat(rel)
  if (exact) return { bucket, keys: [key] }
  const out = []
  let ContinuationToken
  do {
    const page = await s3().send(new ListObjectsV2Command({
      Bucket: bucket, Prefix: key.replace(/\/$/, '') + '/', ContinuationToken,
    }))
    for (const o of page.Contents || []) out.push(o.Key)
    ContinuationToken = page.IsTruncated ? page.NextContinuationToken : undefined
  } while (ContinuationToken)
  return { bucket, keys: out }
}

/**
 * Delete a file, or every object under a folder. DESTRUCTIVE and not
 * recoverable — Vision has no versioning or trash, so a deleted master is gone.
 * The route above it requires the user to type the name to confirm.
 * Returns { deleted, keys } so the caller can report what actually went.
 */
export async function visionDelete(rel) {
  if (PROTO !== 's3') throw new Error('visionDelete requires the s3 protocol')
  const { bucket, keys } = await keysUnder(rel)
  ...
  for (let i = 0; i < keys.length; i += 1000) {
    ...
    const errs = r.Errors || []
    if (errs.length) throw new Error(`${errs.length} object(s) failed to delete, first: ${errs[0].Key} — ${errs[0].Message}`)
  }
  return { deleted: keys.length, keys }
}

/**
 * Rename/move a file or a whole folder. S3 has no rename: every object is
 * copied to the new key and the original deleted, so a folder rename costs one
 * copy per object. The copy is server-side (no bytes through us), but it is NOT
 * atomic — a failure part-way leaves both, which is the safe direction.
 */
export async function visionRename(fromRel, toRel) {
  if (PROTO !== 's3') throw new Error('visionRename requires the s3 protocol')
  const { bucket, keys } = await keysUnder(fromRel)
  if (!keys.length) throw new Error('Nothing to rename at that path')
  const from = s3Parse(fromRel)
  const to = s3Parse(toRel)
  if (!to.bucket || !to.key) throw new Error('Invalid destination')
  if (to.bucket !== bucket) throw new Error('Moving between buckets is not supported')

  const fromBase = from.key.replace(/\/$/, '')
  const toBase = to.key.replace(/\/$/, '')
  if (toBase === fromBase) throw new Error('Destination is the same as the source')
  if (toBase.startsWith(fromBase + '/')) throw new Error('Cannot move a folder inside itself')

  // Refuse to clobber: check the whole destination set before writing anything.
  for (const key of keys) {
    const destKey = key === fromBase ? toBase : toBase + key.slice(fromBase.length)
    if (await visionStat(`/${bucket}/${destKey}`)) throw new Error(`Destination already exists: ${destKey}`)
  }
  ...
  for (const key of keys) {
    ...
    await s3().send(new CopyObjectCommand({
      ...
      // CopySource is a URL path: it must be encoded, or keys with spaces,
      // "&" or "+" (Gallo has plenty) fail or land under a mangled name.
      CopySource: encodeURI(`/${bucket}/${key}`).replace(/\+/g, '%2B'),
    }))
    moved.push({ from: key, to: destKey })
  }
  // Only once every copy has landed.
  for (let i = 0; i < keys.length; i += 1000) {
    await s3().send(new DeleteObjectsCommand({
      Bucket: bucket, Delete: { Objects: keys.slice(i, i + 1000).map(Key => ({ Key })), Quiet: true },
    }))
  }
...
/**
 * Every object under a folder, as Map('/bucket/key' → size). One paginated
 * listing instead of a stat per file — this is what makes the upload tab's
 * preview fast on big folders.
 */
export async function visionListKeys(rel) {
  if (PROTO !== 's3') throw new Error('visionListKeys requires the s3 protocol')
  const { bucket, key } = s3Parse(rel)
  if (!bucket || !key) throw new Error('Not a folder path')
  const out = new Map()
  let ContinuationToken
  do {
    const page = await s3().send(new ListObjectsV2Command({
    ...
    for (const o of page.Contents || []) out.set(`/${bucket}/${o.Key}`, o.Size)
    ContinuationToken = page.IsTruncated ? page.NextContinuationToken : undefined
  } while (ContinuationToken)
  return out
}
