...
export async function copyArtwork(from, to, cat, opts = {}) {
  ...
  if (to === 'gallo') {
    ...
    // Gallo and MadStreamer each issue their own GMVi serials into the same
    // artwork/ folder. If MadStreamer already owns this number for a different
    // album, writing it would replace that album's cover on the website.
    const owner = await findStreamerArtworkByGmvi(ref)
    const norm = c => F(c).replace(/\s+/g, '').toUpperCase()
    if (owner && owner.catalogue && norm(owner.catalogue) !== norm(cat)) {
      throw new Error(`Gallo's ${ref} is also MadStreamer's cover number for ${owner.catalogue} — not copied, it would replace that album's website cover`)
    }
  ...
  if (to === 'streamer') {
    // MadStreamer is the website, so its cover goes through the one pipeline
    // (GMVi JPEG + site sizes, MAM and Tape Files both repointed). This used to
    // write artwork/<GMVi>.jpg directly: no JPEG conversion, no stamped key for
    // a replacement, and MAM and Tape Files never heard about it (2026-09-15).
    const c = await setAlbumCover(cat, buffer, { label: `${from} cover` })
    return { ok: true, to, ref: c.gmvi, key: c.key, url: c.url, mam: c.mam, madstreamer: c.madstreamer,
             wrote: [c.key, `MAM ${c.mam.ok ? '✓' : '✗ ' + (c.mam.reason || '')}`,
                     `MadStreamer ${c.madstreamer?.ok ? '✓' : '✗ ' + (c.madstreamer?.reason || '')}`] }
  }
