...
export const MAP_ALBUM_ON_SONG = {
...
export async function publish(albumID, { visibility = 'Show', onProgress = () => {} } = {}) {
  ...
  try {
    ...
    if (pre.artwork !== 'on s3') {
      ...
      artUrl = made.url
      assertPipelineCoverUrl(artUrl, 'publish: MAM Albums Artwork_S3_URL')
    ...
    assertPipelineCoverUrl(artUrl, 'publish: MADStreamer Tape Files Artwork_S3_URL')
