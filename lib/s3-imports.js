...
export async function uploadPodcastArtwork(buffer, showTitle, ext = '.jpg', contentType = 'image/jpeg') {
  const key = `artwork/podcast-${podcastSlug(showTitle)}${_normExt(ext)}`
  await new Upload({
    client: s3,
    queueSize: 4,
    partSize: 5 * 1024 * 1024,
