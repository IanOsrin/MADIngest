...
/**
 * Every key under an arbitrary prefix in the bucket. Used to find the highest
 * allocated artwork code so a FileMaker-held counter can be re-synced from what
 * is actually in the bucket (the bucket is the source of truth, not the counter).
 */
...
/** artwork/<GMVi>-<YYYYMMDD-HHmmss>.<ext> — the key a REPLACEMENT cover lands on. */
export function artworkKeyForGmviStamped(gmvi, ext = '.jpg') {
  return `artwork/${gmvi}-${_artStamp()}${_normExt(ext)}`
}

// Matches the bare legacy key (artwork/GMVi5276.jpg) and stamped keys
// (artwork/GMVi5276-20260908-101500.jpg) for THIS GMVi — and nothing else.
// The anchor matters: a loose `artwork/GMVi527` prefix also matches GMVi5270
// and GMVi5276, which would hand back another album's cover.
function _artworkKeyRe(gmvi) {
  const safe = String(gmvi).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  return new RegExp(`^artwork/${safe}(-\\d{8}-\\d{6})?\\.(jpg|jpeg|png|webp)$`, 'i')
}

/**
 * Every artwork master belonging to a GMVi, NEWEST FIRST.
 *
 * Sorting is by the stamp in the key, not by S3 LastModified: the stamp is what
 * the published URL is built from, so ordering by it can never disagree with
 * which cover FileMaker points at. A bare unstamped key sorts last — it is by
 * definition the original upload, predating any replacement.
 */
export async function listArtworkKeysForGmvi(gmvi) {
  return selectArtworkKeys(gmvi, await listKeysWithPrefix(`artwork/${gmvi}`))
}

/** The pure half of listArtworkKeysForGmvi — filter + newest-first sort. */
export function selectArtworkKeys(gmvi, keys) {
  const re = _artworkKeyRe(gmvi)
  return keys.filter(k => re.test(k)).sort((a, b) => {
    const sa = a.match(/-(\d{8}-\d{6})\./)?.[1] || ''
    const sb = b.match(/-(\d{8}-\d{6})\./)?.[1] || ''
...
/**
 * @param {object}  [opts]
 * @param {boolean} [opts.stamped=false]  put the cover on a timestamped key.
 *
 * Pass `stamped` when REPLACING a cover that already exists. Overwriting the
 * bare key does not reach the site: the media-edge Worker pins any 200 for 30
 * days at the edge AND in every viewer's browser, so it keeps serving the old
 * image and never re-checks S3 (Dan Hill / GMVi5276, 2026-09-08 — the master
 * was replaced on 4 Sep, the resizer wrote correct derivatives on 5 Sep, and
 * the site still showed the previous cover four days later; 36 covers were
 * stale that way). A stamped key is a URL nothing has ever cached, so the new
 * cover is live the instant FileMaker points at it — the same reasoning as
 * uploadPlaylistArt below.
 *
 * A FIRST upload keeps the bare artwork/<GMVi>.<ext> key on purpose: nothing
 * has cached it yet, so there is nothing to defeat, and the several tools that
 * probe that exact path (artwork-compare, publish-album, the S3 File Checker)
 * keep working unchanged. Only replacements need a new URL.
 *
 * The caller MUST write the returned url to FileMaker — with a stamped key the
 * URL is no longer derivable from the GMVi, so a cover nobody repoints is a
 * cover nobody sees.
 */
export async function uploadArtworkByGmvi(buffer, gmvi, ext = '.jpg', contentType = 'image/jpeg', { stamped = false } = {}) {
  const key = stamped ? artworkKeyForGmviStamped(gmvi, ext) : artworkKeyForGmvi(gmvi, ext)
  await new Upload({
    client: s3,
    queueSize: 4,
...
export async function presignAudioDownload(s3url, filename) {
  ...
  const cmd = new GetObjectCommand({
    Bucket: bucket,
    Key: key,
    // No filename → inline streaming (audition playback); with filename →
    // attachment download (Download Track tab).
