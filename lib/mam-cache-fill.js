 ...
 */
import { lookupAlbumTracks, getStatus } from './metadata-cache.js'
 ...
 */
export async function applyCacheFill(catalogue, { acceptSuggestions = [], acceptConflicts = {}, skipFills = false } = {}) {
 ...
 */
export async function addMissingSong(catalogue, title) {
  const cat = String(catalogue || '').trim()
  const want = tn(title)
  if (!cat || !want) throw Object.assign(new Error('catalogue and title are required'), { status: 400 })

  const plan = await planCacheFill(cat)
