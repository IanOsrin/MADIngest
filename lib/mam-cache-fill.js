...
const conflictsWith = (fills, fieldData) => Object.fromEntries(
  Object.entries(fills)
    ...
    .filter(([k, v]) => tn(v) !== tn(fieldData[k]) && !sameValue(v, fieldData[k]))   // 0:02:28 = 00:02:28
  ...
  try {
    ...
    for (const s of songs) {
      ...
      if (best && bestScore >= suggestAbove) {
        ...
        const conflicts = conflictsWith({ 'Track Name': nn(best.track_name), ...all }, g)
    ...
    const note = seqNote || (!cacheRows.length
