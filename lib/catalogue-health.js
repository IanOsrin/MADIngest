 ...
 */
/**
 * fixFields — the MadStreamer fields the Data Health drill-in may edit for a
 * check. Absent means "report only".
 *
 * Only fields where the correction IS a value: no cover or audio checks, whose
 * fix is a file rather than a keystroke, and nothing on hidden-from-site, which
 * is a composite of several other checks and would invite editing the symptom.
 *
 * These names are FileMaker's, not the mirror's column aliases — the edit goes
 * to MadStreamer via updateStreamerRecord, because the mirror is READ-ONLY (a
 * nightly copy). A fix therefore will not move these counts until the next
 * sync, and the UI has to say so.
 */
export const CHECKS = [
  {
    code: 'isrc-shared', fixFields: ['ISRC'], severity: 'error', group: 'Identity',
  ...
  {
    code: 'code-junk', fixFields: ['ISRC', 'UPC'], severity: 'error', group: 'Identity',
  ...
  {
    code: 'isrc-missing', fixFields: ['ISRC'], severity: 'warn', group: 'Identity',
    label: 'No ISRC',
  ...
  {
    code: 'upc-missing', fixFields: ['UPC'], severity: 'warn', group: 'Identity',
  ...
  {
    code: 'catalogue-split-title', fixFields: ['Album Title'], severity: 'warn', group: 'Album coherence',
  ...
  {
    code: 'catalogue-split-artist', fixFields: ['Album Artist'], severity: 'info', group: 'Album coherence',
  ...
  {
    code: 'repeated-track', fixFields: ['Track Name', 'Sequence Number'], severity: 'warn', group: 'Album coherence',
  ...
  {
    code: 'repeated-sequence', fixFields: ['Sequence Number'], severity: 'warn', group: 'Album coherence',
  ...
  {
    code: 'album-title-missing', fixFields: ['Album Title'], severity: 'warn', group: 'Album coherence',
  ...
  {
    code: 'no-sequence', fixFields: ['Sequence Number'], severity: 'info', group: 'Album coherence',
  ...
  {
    code: 'no-catalogue', fixFields: ['Album Catalogue Number'], severity: 'warn', group: 'Album coherence',
...
export async function getHealthRecords(code, { limit = 500, offset = 0 } = {}) {
  ...
  return {
    ...
    // Which fields the drill-in may edit for this check — absent means report-only.
    fixFields: check.fixFields || [],
...
/**
 * Every track of one catalogue, in track order.
 *
 * The album-level checks — two album titles on one catalogue, a repeated track
 * number — flag a CATALOGUE, not a row, so there is nothing for the drill-in to
 * edit. This expands one of them into its tracks, which the same per-field edit
 * then applies to: you see both titles side by side and correct the odd one.
 *
 * Matched on the normalised catalogue (CATN), the same key the checks group by,
 * so what comes back is exactly the set that was flagged — spacing and
 * punctuation differences included.
 */
export async function getCatalogueRecords(cat) {
  if (!isMirrorEnabled()) return { ok: false, reason: 'mirror not configured' }
  const c = String(cat || '').trim()
  if (!c) return { ok: false, reason: 'catalogue required' }
  const norm = c.toUpperCase().replace(/[^A-Za-z0-9]/g, '')
  if (!norm) return { ok: false, reason: 'catalogue required' }
  const r = await mirrorQuery(
    `SELECT fm_record_id,
            raw->>'Sequence Number' AS "Sequence Number",
            raw->>'Track Name'      AS "Track Name",
            raw->>'Album Title'     AS "Album Title",
            raw->>'Album Artist'    AS "Album Artist",
            raw->>'ISRC'            AS "ISRC",
            raw->>'UPC'             AS "UPC",
            ${CAT} AS "Album Catalogue Number"
       FROM tracks
      WHERE ${CATN} = $1
      ORDER BY nullif(regexp_replace(coalesce(raw->>'Sequence Number',''), '\\D', '', 'g'), '')::int NULLS LAST,
               raw->>'Track Name'
      LIMIT ${RECORD_CAP}`, [norm])
  return {
    ok: true, catalogue: c, rows: r.rows,
