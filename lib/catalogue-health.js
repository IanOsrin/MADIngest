...
export const CHECKS = [
  {
    ...
    label: 'One ISRC on more than one song',
    why: 'ISRC identifies a recording. Royalty reporting and DSP delivery key on it, so plays are credited to the wrong track. It also makes the songs collapse into one row anywhere they are matched by ISRC.',
    countSql: `
      SELECT count(*)::int AS n FROM (
        SELECT raw->>'ISRC' i FROM tracks
         WHERE coalesce(raw->>'ISRC','') <> '' AND raw->>'ISRC' !~* ${JUNK}
         GROUP BY 1 HAVING count(DISTINCT ${NORM("raw->>'Track Name'")}) > 1) x`,
    unit: 'ISRCs',
    recordsSql: `
      WITH bad AS (
        SELECT raw->>'ISRC' i FROM tracks
         WHERE coalesce(raw->>'ISRC','') <> '' AND raw->>'ISRC' !~* ${JUNK}
         GROUP BY 1 HAVING count(DISTINCT ${NORM("raw->>'Track Name'")}) > 1)
      SELECT fm_record_id, raw->>'ISRC' isrc, raw->>'Track Name' title,
             raw->>'Album Title' album, raw->>'Album Artist' artist, ${CAT} cat
        FROM tracks WHERE raw->>'ISRC' IN (SELECT i FROM bad)
...
const ISRC_SWEEP = `
  SELECT count(*)::int AS "isrc-shared" FROM (
    SELECT raw->>'ISRC' FROM tracks
     WHERE coalesce(raw->>'ISRC','') <> '' AND raw->>'ISRC' !~* ${JUNK}
     GROUP BY 1 HAVING count(DISTINCT ${NORM("raw->>'Track Name'")}) > 1) x`
